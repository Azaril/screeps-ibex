...
- **Keep shared abstractions generic — no usage-specific bleed.** The `Job`/`Mission`/`Operation` traits and their dispatch enums (`JobData`/`MissionData`/`OperationData`) must expose only *truly generic* surface. Do not add one-off, caller-specific predicates or helpers to them (anti-example: an `is_power_bank_attack()` method on the generic operation surface). Instead, in preference order: (1) keep the information **with its owner** — the module that created the thing usually already has the data (e.g. the war coordinator launched the attack with an `AttackReason` and can record it in its own bookkeeping rather than asking the operation afterward); (2) put concrete accessors on the **concrete type** and `match` the dispatch-enum variant at the caller that owns the strategy; (3) where multiple types genuinely share a capability, define a small **capability trait** (e.g. `HasAttackReason`) implemented opt-in by the types that have it — never widen the base trait. A generic surface accreting `is_<specific-thing>` methods is the smell to reject in review.
