...
static COUNTS: [AtomicU32; CATEGORY_COUNT] = [
    AtomicU32::new(0),
    AtomicU32::new(0),
    AtomicU32::new(0),
    AtomicU32::new(0),
...
/// Chained FNV-1a over the tick's intent tuples (order-sensitive).
static DIGEST: AtomicU64 = AtomicU64::new(FNV_OFFSET);
...
/// Reset at tick start (`metrics::tick_start`).
pub fn reset_tick() {
    for count in &COUNTS {
        count.store(0, Ordering::Relaxed);
    }
    DIGEST.store(FNV_OFFSET, Ordering::Relaxed);
}

/// Fold one issued intent into the tick's counts + digest.
pub fn record(category: IntentCategory, creep_name: &str, target_pos: Option<Position>) {
    COUNTS[category as usize].fetch_add(1, Ordering::Relaxed);
    let mut hash = DIGEST.load(Ordering::Relaxed);
    ...
    DIGEST.store(hash, Ordering::Relaxed);
}

/// The tick's recorded view, for the seg-57 block.
pub fn snapshot() -> ([u32; CATEGORY_COUNT], u64) {
    let mut counts = [0u32; CATEGORY_COUNT];
    for (slot, count) in counts.iter_mut().zip(COUNTS.iter()) {
        *slot = count.load(Ordering::Relaxed);
    }
    (counts, DIGEST.load(Ordering::Relaxed))
}
...
pub fn attack<T>(creep: &Creep, flags: &mut SimultaneousActionFlags, target: &T, target_pos: Position) -> bool
where
    T: ?Sized + Attackable,
{
    ...
    record(IntentCategory::Attack, &creep.name(), Some(target_pos));
    let _ = creep.attack(target);
    true
}

/// Pipeline B ranged attack.
pub fn ranged_attack<T>(creep: &Creep, flags: &mut SimultaneousActionFlags, target: &T, target_pos: Position) -> bool
where
    T: ?Sized + Attackable,
{
    ...
    record(IntentCategory::RangedAttack, &creep.name(), Some(target_pos));
    let _ = creep.ranged_attack(target);
    true
}

/// Pipeline B ranged mass attack.
pub fn ranged_mass_attack(creep: &Creep, flags: &mut SimultaneousActionFlags) -> bool {
    if !flags.consume(SimultaneousActionFlags::RANGED_MASS_ATTACK) {
        return false;
    }
    record(IntentCategory::RangedMassAttack, &creep.name(), None);
    let _ = creep.ranged_mass_attack();
    true
}

/// Pipeline C heal.
pub fn heal<T>(creep: &Creep, flags: &mut SimultaneousActionFlags, target: &T, target_pos: Position) -> bool
where
    T: ?Sized + Healable,
{
    ...
    record(IntentCategory::Heal, &creep.name(), Some(target_pos));
...
pub fn ranged_heal<T>(creep: &Creep, flags: &mut SimultaneousActionFlags, target: &T, target_pos: Position) -> bool
where
    T: ?Sized + Healable,
{
    ...
    record(IntentCategory::RangedHeal, &creep.name(), Some(target_pos));
