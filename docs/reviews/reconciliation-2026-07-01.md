...
- **Tranche 1 — implementing now** (safe, self-contained, adversarially reviewed): **REC-062** proper (structure-progress signal — the correct fix for the deferred turtle disengage); **0031 §5 FormationShape 5–8** (2×2 overlay silently wrong — a real correctness bug); **0030 §10 D16** (delete dead `bodies.rs` template sizing) + **D19** (lock-in test `no_dynamic_doctrine_silently_fields_static`).
- **[CODE] Tranche 2 (safe, follow-up — greenlit for a later batch):** 0030 §10 D12/D13/D14/D15/D17 (silent-static fallbacks, member_energy:0, Sizing enum, min_count floor, defend-flag→GarrisonDefense); 0031 §5 budget-free `emit_requirement` + #38 escalate-vs-abandon-on-`assemble_force=None`; 0028 K3 `slots_to_spawn`/K4 `claims_allowed` bot wiring; 0025 §11 #12 `member_intents` bot wiring + #11 Declaim-in-decision-view + action_oscillation_rate metric; plan §5 U-TOWER `decide_towers` + W3 Escort producer + K2c-2 yield-to-defense predicate + K-RECONCILE; 0034 §3 objective-level abort + D4-F1 recycle job; V-1 hygiene (room-filter `ev_members` + debug_assert); 0008a T0.1/T0.2/T1.3–1.6 (reachable-heal filter, net-repair sizing, MOVE back-load, CLAIM-attacker priority, RMA shield-check, exit-tile discipline).
