...
- **[CODE] Tranche 2 — IN PROGRESS (operator: WFV bumps fine, clean design, no tech debt — [[wfv-fine-clean-design-no-debt]]).** **DONE:** **V-1 hygiene** (ev_members room-filter + debug_assert, committed `b4c7c99`); **plan §5 U-TOWER** `decide_towers` — done CLEANLY in combat-decision (harness-validatable), squad-DPS split-fire, focus-id plumbing through SquadContext (**WFV 25→26**), U7 combined-fire harness scenario, 5 MINORs cleared (shared `threat_value` + rampart filter, honest ordering docstring, rename, behavioral lag test); **plan §5 W3 Escort producer** — activates the inert `Escort{room}` for threatened marginal claims (viability/threat-only guards, `ESCORT_OBJECTIVE_TTL`, producer test). **Sub-batch 2 (sizing/doctrine, adversarially reviewed, both PASS):** **D15 min_count floor DONE** (explicit `MIN_SIZED_MEMBERS`/`min_members` on CompositionParams, argmax rejects sub-floor candidates → honest None, default 1 = byte-identical); **#38 escalate-vs-abandon DONE** (the silent `assemble_force=None` drop is now a LOUD+counted ABANDON via `mark_unwinnable` + ESCALATE via re-scout registration, `clear_unwinnable` recovers). **D14/D12/D13/D17 = NOT-A-DEFECT (already resolved by the ADR 0031 rewrite, file:line-verified):** `is_sized()` was DELETED not enum-replaced (every doctrine is dynamic through one driver; the distinction is `honor_verdict()`); no silent-static combat-body fallback remains (all fielding uses explicit `else { continue }`); every defense arm passes real home energy (the `0` is a per-home-overwritten placeholder; genuine-0 → honest None); the defend-flag already routes through `GarrisonDefense`. **budget-free `emit_requirement` → RECLASSIFIED [ADR-deferred]:** `optimizer_ceiling_budget` is NOT vestigial — it's the winnability-ceiling injection seam the eval's FN-falsifier is deliberately coupled to; retiring it IS the Deferred §5 assess/clear_force redesign (changes calibration), not a quick cleanup. **Sub-batch 3 (bot wiring, verify-first, adversarially reviewed):** **DONE:** 0025 §11 #12 **member_intents bot wiring** (kernel per-creep intents now consumed by the SquadCombatJob action path via `translate_intents`, closing the sim-vs-bot action divergence; ephemeral serde-skip, no WFV); **K2c-2 yield-to-defense** (hardcoded `military_free:true` → real `militarily_active` predicate; a defended base pauses a new SK farm); **0034 §3 D4-F1 recycle terminal** (`MemberTravelAction::Recycle` + `TickMovement::Recycle` — the Recycle verdict was held-forever, now recycles at home; never abandons a fighter); **0008a T0.2** breach-repair parity; **T1.5** RMA shield-check pins. **NOT-A-DEFECT/DEFERRED (evidence-backed):** K3 already wired to its eval consumer; K4 DEFERRED (wiring the single-scalar kernel would regress the bot's REC-008 defense-exempt forming asymmetry); Declaim-in-decision-view DEFERRED (real ADR §11 #11 increment — needs a downgrade-block-timer model + `V_controller`; the dedicated Declaim objective handles it live); action_oscillation_rate DEFERRED (needs a harness A-B-A tracker + baseline-calibrated gate, now unblocked by member_intents); K-RECONCILE DEFERRED (cross-mission refactor gated on G4-defense); 0034 Phase-1.5 fast-abort DEFERRED. **REVERTED:** 0008a T1.3 MOVE back-load (measurably regressed two combat-eval gates — pulled, not shipped). **Tranche 2 [CODE] = COMPLETE** (all tractable items done; the rest are ADR/gated, tracked below).
