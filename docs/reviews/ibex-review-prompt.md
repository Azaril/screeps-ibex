...
- **Simulation / replay harness**: record real state (rooms, creeps, memory) from the private server and replay offline to reproduce bugs deterministically and regression-test strategy. Consider `screeps-server-mockup` / a local private server for integration runs.
...
- **`screeps-game-api` fork** — `C:\code\screeps-game-api` (upstream: rustyscreeps/screeps-game-api) — consult for the exact semantics of `resolve()`, `RawMemory`, pathfinder, store, and market APIs used in hot paths.
