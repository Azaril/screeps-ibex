...
- **Status:** Accepted; **ARCHITECTURE COMPLETE on `master`** (queue + manager + tactics, 2026-06-18; anchor mover / formation travel / breach orientation O1–O3 landed through 2026-06-26). **M4 "Combat-Effective" is a LIVE-SOAK VALIDATION milestone** (see [`../plans/combat-overhaul-plan.md`](../plans/combat-overhaul-plan.md)), not code delivery. **H4/H5 harness completeness** (ADR 0028 K3/K4) and **I1/I2 identity** remain the only open code items. Live status: [`../plans/combat-overhaul-plan.md`](../plans/combat-overhaul-plan.md) (master status/plan doc).
...
| **4** | **`CombatObjectiveQueue` + `SquadManager` + tactics; migrate `AttackMission`→producer; delete the legacy** | `military/objective_queue.rs` + `military/squad_manager.rs`; `war.rs` produces objectives; `attack_mission.rs`/`attack.rs` deleted | Memory/format + Behavioral — **✅ DONE (2026-06-18).** Queue+manager+tactics (G1/G2/G3/G3-tail), full offense migration (G4-O1 travel / O2 orient / O3 breach / O4 wave-retry / O6 producers `InvaderCore`→`Dismantle`, `AttackFlag`→`Secure`, `ResourceDenial`→`Harass`), and **O7 = legacy deletion** (`AttackMission`/`AttackOperation`/`AttackReason` gone, WFV 12→13). Power-bank dropped (O5, was non-functional → deferred §5(g)); heavy multi-squad assault deferred (§5(g)). |
| **5** | **Migrate defense + escort onto the queue**; `WarOperation` becomes a supervisor (withdraw/trim — IBEX-026/028); add `UnwinnableTarget` backoff | `war.rs`, `squad_defense.rs` removed, `claim.rs` escort producer | Behavioral — **defense ✅ DONE (P2.G4-defense: `squad_defense.rs` removed, all defense → `Defend` objectives); `UnwinnableTarget` backoff DONE (O4). Escort producer (W3) + supervisor trim/economy-abort (W2) + `WarDecl` posture (W4) still TODO.** |
| **6** | **Synchronized spawning** via `GroupId`/align-finish/pre-spawn; boost handoff (or kill-switch off) | `spawnsystem.rs` executor unchanged; ADR 0011 D3/D4/D9 | Behavioral — **TODO (S1/S2). Gated on `SquadId` (I1/I2); S2 boost handoff gated on the ADR 0010 lab pipeline.** |
...
### Current state & roadmap to completion (2026-06-18; updated 2026-07-01)

> Live, dependency-ordered status is the [master plan doc](../plans/combat-overhaul-plan.md) §3–5 (the single source of truth that supersedes this table for *status*). This section pins the architectural roadmap to the **M4 "Combat-Effective"** exit milestone so the ADR records the end-to-end path.

> **CURRENT STATE (2026-07-01):** The **architecture is complete and deployed to MMO** — squads reach, rally, engage, and hold; structure/tower-defense targeting and rally/engage gating are done. Since the 2026-06-18 snapshot below, the follow-on decisions **ADR 0027/0031/0032/0034/0035/0036/0037** landed on `master` (auction / rally-oscillation + reach fixes / force-sizing oracle / squad reassignment / defense targeting — see the master plan doc and those ADRs). **M4 is now purely a live-soak validation milestone**, not a code-delivery gate. The only remaining *code* items are **H4/H5 harness completeness** (scenarios/opponents/self-play/replay + server-parity oracle — ADR 0028 K3/K4) and **I1/I2 identity** (`SquadStore`/minted `SquadId`). The war-supervision and synchronized-spawning tracks listed in the 2026-06-18 roadmap below have since been subsumed by the auction/force-sizing/reassignment work — treat those track entries as historical, not open.

**Done — the architecture is in place and running on `master` (Docker WFV 13; deployed to MMO).** The decision's core is fully realized: the `CombatObjectiveQueue` (G1), the `SquadManager` lifecycle owner (G2), the pure tactics engine (`decide_squad`/`decide_combat`/`decide_movement` in `screeps-combat-decision` — focus-fire, coupled engage/retreat hysteresis, cohesive pathfinding-scored kiting, heal assignment, formation orientation, breach-aware dismantle; G3/G3-tail), the anchor mover (M1/M2/M3), and the seg-57 cohesion metrics + colony-health military term (H1–H3). **All combat — offense and defense — is now objective-driven through one manager** (defense → `Defend`; offense → `Secure`/`Dismantle`/`Harass`); the legacy `AttackMission`/`AttackOperation`/`AttackReason` is **deleted** (O7). Source-keeper farming runs on the same model (K-workstream). **(2026-07-01) The live combat path IS now behaviorally exercised and deployed to MMO** — with the ADR 0027/0031/0032/0034/0035/0036/0037 follow-ons landed, squads reach/rally/engage/hold, structure + tower-defense targeting and rally/engage gating work end-to-end. The 2026-06-18 "peaceful soak, combat path not yet exercised" gap is **closed**; M4 remains open only as ongoing live-soak validation. The historical 2026-06-18 snapshot ("Last Docker soak: clean WFV-13 reset, CPU bucket full, ~101 creeps / 6 rooms, peaceful") is retained below for provenance.

**Roadmap to M4 (2026-06-18 snapshot — retained for provenance; see the 2026-07-01 CURRENT STATE note above for what has since landed):**
1. **Behavioral validation of the live combat path** — the bot must clear a real invader core (`Dismantle`) + hold a defended room (`Defend`) cohesively on the private server, no CPU spiral / orphan-idle. Needs a *threat* (peaceful soak can't show it): either wait for one to arise, introduce one, or drive it via **H4** (scenarios/opponents/self-play harness). This is the gate that flips CP-H/CP-M from code-done to milestone-met.
2. **Harness completion (H4 → H5)** — scenarios + scripted opponents + self-play runner + replay (H4), then the server-parity oracle + nightly seeded gate + byte-exact golden vectors (H5). Unblocks the **ADR 0008a EXP-\* tactics-tuning loop** (turning the catalog from hypotheses into shipped defaults) and the live military-score check.
3. **Identity (I1 → I2)** — `SquadStore` + minted `SquadId` replacing the interim `SquadContext`-Entity keying (ADR 0001 A2); closes the dangling-ref class and gates synchronized spawning + a future multi-squad assault.
4. **War supervision (W2/W3/W4)** — `WarOperation` trims/economy-aborts on real per-squad spend (IBEX-026/028), the `Escort{room}` claim pre-clear producer (the kind exists but is inert), and the thin `WarDecl` posture hook (ADR 0014).
5. **Synchronized spawning (S1/S2)** — manager as the sole demand producer with `GroupId = SquadId` + align-finish + pre-spawn (ADR 0011), then boost-on-spawn handoff behind the kill-switch (gated on the ADR 0010 lab pipeline). Depends on I1/I2.
6. **Source-keeper finish (K2c-2 / K-RECONCILE)** + the deferred **K4** mineral mining (blocked on remote extractor/container construction).

**Deferred (out of the M4 critical path, tracked in the master doc §5(g)):** power-bank farming (its own ADR + healed squad + predictive collector), the heavy multi-squad player assault (needs a multi-squad/sequenced-objective mechanism + I1/I2 + `attack_players`), the cross-room flee (L1), and the trait-based lazy combat view (L2, CPU-gated). **M4 "Combat-Effective" is reached** when track 1 validates on the server and CP-I/CP-G/CP-W/CP-S/CP-K + the 11 M4 exit criteria close.
