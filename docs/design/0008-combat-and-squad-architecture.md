...
Surfaced by a live incident: an established but **towerless RCL2 room (W11N57)** was being declaimed by an enemy **CLAIM creep** with no defender ever spawning. Fixing it end-to-end on the pre-manager `SquadDefenseMission`/`bodies.rs` path produced combat-system learnings the SquadManager (the Decision above) MUST carry forward — these are properties of *body sizing* and the *spawn/threat seam*, independent of whether lifecycle lives in a mission or the manager.

1. **The defense trigger must key on threat by body parts, not just combat parts.** `RoomDynamicVisibilityData::hostile_creeps()` flags only ATTACK/RANGED_ATTACK/WORK, so an enemy CLAIM creep neutralising the controller (or a lone dismantler/healer) was invisible to the scan and no defense mission was created. The trigger now also fires on `hostile_threat_creeps()` and commits a defender only to hostiles bearing a "worth-defending" part (Attack/RangedAttack/Work/Claim/Heal) — `operations/war.rs` `hostile_warrants_defender`. The manager's objective trigger must inherit this: a controller-attacker is a defendable threat.

2. **A body's MINIMUM buildable cost must be affordable at the lowest RCL the role serves.** `solo_defender_body`'s repeat unit `[RangedAttack, Move, Heal, Move]` (500e, `minimum_repeat:1`) made `create_body` return `Err` below 500e, so a young RCL2 room (capacity < 550, often < 500 before all extensions) spawned nothing and the room was declaimed undefended. The 250e HEAL forced into the minimum was the culprit. **Rule: never force an expensive part (HEAL/TOUGH) into the minimum repeat — size it as a fixed, affordability-gated addition.** Same latent bug in `duo_healer_body` (660e fixed TOUGH/MOVE front ⇒ no healer below RCL3).

3. **Threat-matched sizing, built as a direct `Vec<Part>`.** New pure helpers in `military/damage.rs` — `attack_parts_to_kill`, `defender_heal_parts_for_dps`, `defender_spawn_readiness` — turn `RoomThreatData` (estimated_dps/heal, worst-target HP, boosted) + room energy into a body. The sized builders (`military/bodies.rs` `sized_defender_body`/`sized_healer_body`) construct the final `Vec<Part>` directly (offense → HEAL when affordable → TOUGH when affordable → MOVE, degrading to a `[RangedAttack, Move]` 200e floor), **bypassing `SpawnBodyDefinition`'s `&'static [Part]` repeat-template constraint** — exact composition, no static-array hacks. The manager's composition→roster step should size members this way rather than from fixed `BodyType` templates at `energy_capacity`.

4. **Offense must out-damage the AGGREGATE enemy heal (focus-fire), not per-target self-heal.** Hostiles heal each other and concentrate healers on the creep under fire, so `attack_parts_to_kill` takes `enemy_focus_heal = RoomThreatData.estimated_heal`. When one defender can't out-DPS it within the kill window (`None`), the answer is to **escalate count** (Duo/Quad stack DPS and focus-fire one target), not build a bigger solo — exactly the manager's `ForceRequirement` knob.

5. **Spawn-now-vs-wait needs `energy_available` (current) vs `energy_capacity_available` (max).** Sizing only off capacity makes a capable room on a momentary energy dip stall; sizing off available emits a runt. `defender_spawn_readiness` returns `Wait` for a capable room that just needs to refill (a defender is already holding, or a tower buys time), but `SpawnNow(available)` when nothing is holding the line in a towerless room — a smaller defender now beats a perfect one too late. The manager's pre-spawn/spawn step should apply this gate. (Complements §2.3's pre-spawn: *when* to spawn from current vs full energy, distinct from *when* to pre-spawn a successor.)

6. **Multi-room sourcing via one shared spawn token.** A defender is sourced by broadcasting ONE `spawn_queue.token()` to every in-range home room (linear distance ≤ `MAX_DEFENSE_SOURCE_DISTANCE`); the shared `spawned_tokens` set in `SpawnQueueSystem` fulfils it at most once and leaves it pending if the first room can't afford it. Over-spawn-free, resilient sourcing — the pattern the manager's reconciler (§2.2 wishlist) should use for shortfall requests: one token per needed slot, several candidate rooms.

7. **Owned rooms must be defended against NPC invaders.** The owned-room defense scan previously filtered out *all* NPCs; the separate invader path covers only *reserved* remote rooms. Owned rooms now defend against invaders (Source Keepers still excluded). The manager's defend-objective trigger must not assume player-only threats.

When the SquadManager lands (Step 2), fold the sizing helpers (`damage.rs`) and the `sized_*_body` builders into its composition→roster + pre-spawn steps; do not regress the minimum-affordability (§learning 2), focus-heal (§4), and energy-readiness (§5) properties.
