...
| **4** | **`CombatObjectiveQueue` + `SquadManager`** behind the live offense path (parity), then migrate `AttackMission` to a producer; manager computes tactics; generalize `handle_wave_wipe`; delete combat `request_renew` | new `military/objective_queue.rs` + `military/squad_manager.rs`; `attack_mission.rs` shrinks; `war.rs` produces objectives | Memory/format + Behavioral — **queue+manager+tactics DONE (G1/G2/G3/G3-tail + G4-O1/O2/O3); `handle_wave_wipe`→manager wave-retry/unwinnable = O4 (in progress); migrate `AttackMission`→`Secure`/`Dismantle`/`Farm{PowerBank}` producers + delete = O6/O7 (soak-gated). Full offense checklist: [`../execution/g4-offense-plan.md`](../execution/g4-offense-plan.md).** |
