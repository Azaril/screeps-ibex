...
- **Status:** Accepted — **core architecture IMPLEMENTED** on `master` as of 2026-06-18 (the queue + manager + pure tactics + anchor mover are live; all combat is objective-driven and the legacy `AttackMission`/`AttackOperation` is deleted). Remaining = the harness/identity/war-supervisor/spawn tracks + live behavioral validation → **M4 "Combat-Effective"** (see "Current state & roadmap to completion" below). Live status: [`../plans/combat-overhaul-plan.md`](../plans/combat-overhaul-plan.md) (master status/plan doc).
...
### Current state & roadmap to completion (2026-06-18)
