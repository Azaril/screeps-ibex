...
- **Status:** Decided
- **Date:** 2026-06-09 · **Revised:** 2026-06-16 (overhaul pass: corrected the module inventory; adopted the scout pull model; added the tactics/orders model and the anchor-mover movement decision; pinned harness-first sequencing)
- **Deciders:** William Archbell
- **Related:** Field Report A (war/squad cohesion), Field Report B (operation/mission lifecycle hangs); IBEX-001, IBEX-002, IBEX-002b, IBEX-015, IBEX-026, IBEX-027, IBEX-028, IBEX-029, IBEX-041, IBEX-042. Operator brief 2026-06-16: squads "very ineffective" — *they just stand and ranged-mass-attack* (null tactics), get **orphaned** when an objective completes, sit **idle**, and **scatter** (group pathfinding). The operator's chosen reference is the **scouting** subsystem (request→claim→fulfill→release), and the chosen sequencing is **harness-first**. Sibling ADRs: **0006** (eval harness — hosts the *combat micro-sim* this ADR is validated on; **0008 work is gated behind it**), **0001** (entity model — the marker-converted `squad_entity: EntityOption<Entity>` squad reference), **0003** (behavior modeling — owns the guarded intent sink and the squad-cohesion invariants), **0011** (spawn orchestration — `GroupId` synchronized spawning + pre-spawn), **0014** (empire posture — declares WAR/objectives), **0015** (testing — registers the combat seams), **0017** (threat-aware expansion — defers escort/pre-clear to this ADR). Reference design in-repo: the `VisibilityQueue` scout pull system (`screeps-ibex/src/room/visibilitysystem.rs`). Prior art: Overmind Overlord/Directive + `autoMelee`/`autoRanged`/`autoSkirmish` (`../references/external-references.md`).

> **Scope boundary (read first).** This ADR is the **architecture + tactics** decision for combat: *which* combat objectives exist, the seam between an objective producer, a generic squad manager, the spawn system, and the per-tick tactical orders a squad executes. ADR 0003 owns the squad-cohesion *invariants* (the guarded intent sink, defense-is-never-squad-less, the cohesion force-abort) plus the movement-model analysis that constrains any mover — the mover itself is decided here; ADR 0001 owns squad *identity* (the marker-converted `squad_entity: EntityOption<Entity>`); ADR 0011 owns *spawn scheduling* (`GroupId`/align-finish/pre-spawn); ADR 0014 owns *strategic intent* (the `WarDecl` that gates player offense); ADR 0006 owns the *harness* (the combat micro-sim + cohesion metrics that make this work measurable). This ADR consumes those and decides the combat-specific layering, the goal-management model, and the tactics.
...
1. **The inventory was stale.** `SquadAssaultMission`, `SquadHarassMission`, and `raid.rs` **no longer exist in the tree** — the DELETE rows in the old ADR are moot. The combat reality this ADR starts from is exactly three modules: one `SquadCombatJob` (`jobs/squad_combat.rs`), one offense mission `AttackMission` (`missions/attack_mission.rs`), and one defense mission `SquadDefenseMission` (`missions/squad_defense.rs`). (Looting is economic via `salvage.rs`, not a combat squad.)
...
- **VM-reset resilience.** Squad identity, roster, and especially *assignment* state must survive a reset (IBEX-002b). The scout subsystem solves this by keeping assignment **ephemeral**.
...
> **These five defects are the PRE-OVERHAUL diagnosis** — the motivating problem statement the decision below answers, not a description of the target architecture. They are retained because the shape of the design (and several of its invariants) only make sense against them.
...
Adopt a **scout-style, queue-decoupled combat architecture** with three coordinated pieces, all gated behind the combat harness (ADR 0006) and ADR 0001's marker-converted squad reference (`squad_entity: EntityOption<Entity>`):
...
2. A **`SquadManager`** — a single perpetual ECS system (like `ScoutOperation`) that claims objectives for squads, reconciles desired-vs-live rosters into spawn demand, pre-spawns replacements (never renews), enforces cohesion-as-invariant + deadlines, **computes the per-tick tactical orders**, and retires/retasks squads.
...
WarOperation / AttackOperation / ClaimOperation     strategy: what & why (ADR 0014 posture/WarDecl)
        ...
        ▼
CombatObjectiveQueue   (global resource)             request → claim → complete → release → retire
        │  persistent: durable ObjectiveData + give-up backoff
        │  ephemeral:  assignment (claimed_by), this-tick status  ── NEVER serialized
        ▼  manager claims an objective for a squad
SquadManager   (global system)                       objectives → rosters → spawn demand → ORDERS → retire
        │  reconciles desired vs live; creates the squad; pre-spawn; force-abort; tactics
        ▼  per-tick TickOrders + Follow targets
Squad   (SquadContext entity, ADR 0001 marker ref)   roster + formation + lead/anchor + tick orders
        │  per-member orders
        ▼
...
| Module | Instantiated? | Squad layer before this ADR | Verdict |
|---|---|---|---|
| **`SquadCombatJob`** (`jobs/squad_combat.rs`) | yes — the only per-creep combat job | the executor | **KEEP** — shrinks to pure order execution + a `Recall` escape valve; loses all targeting-of-last-resort once it always has orders. |
| **AttackMission** (`attack_mission.rs`) | yes — `attack.rs` (per wave) | yes — `SquadContext` + virtual anchor | **MERGE.** Becomes an objective *producer* (`Secure`/`Dismantle`/`Harass`/`Farm{PowerBank}`); lifecycle + tactics move to the `SquadManager`, which owns the siege capabilities (anchor mover, threat orientation, breach targeting, wave retry). The legacy mission is deleted once every reason it still carries — power-bank farming and the heavy multi-squad assault — has a home in the new model. |
| **SquadDefenseMission** (`squad_defense.rs`) | ~~`war.rs` defense scan~~ | ~~squad-LESS~~ | **MERGE → REMOVED.** All three `war.rs` defense contexts (owned reactive / defend-flag / remote-invader) upsert `Defend` objectives fielded by the `SquadManager` with the §4 tactics; the file, `MissionData::SquadDefense`, and the `manager_defense` flag are gone. Producer-scoping preserves the ADR 0017 §13 ownership invariant and removes the reserved-remote defense thrash. |
| **DefendMission** (`defend.rs`) | yes — economic missions | no — not a squad mission — a room-safety *signal* (`is_room_safe()`), spawns nothing | **KEEP** (rename-candidate `RoomSafetyMission`). Out of scope — produces no creeps. |
| **NukeDefense / SafeMode / WallRepair** | yes — `war.rs` | no — utility, no squads | **KEEP** as-is. |
...
| Two-layer state: persistent durable facts + **ephemeral** assignment (`claimed_by`) | Persistent: the objective + give-up backoff. Ephemeral (never serialized): which squad claims it — self-heals on reset, **cannot dangle** (kills IBEX-002b for the goal layer). |
...
- `Defend{room}` → `!militarily_active(room)` AND `owner().mine()`. **If the room stops being ours, the objective is withdrawn immediately** (preserves the ADR 0017 §13 ownership-subordinate invariant).
...
- `WarOperation::run_offense_evaluation` → `Secure`/`Dismantle`/`Harass`/`Farm` (player offense gated by ADR 0014 `WarDecl`; NPC policing autonomous). **The offense-producer mapping:** each `AttackReason` maps to one objective kind + a threat-/role-matched composition + a priority, mirroring `run_defense_scan`'s escalation→composition pattern:

  | `AttackReason` (legacy) | → `ObjectiveKind` | Composition | Priority | Notes |
  |---|---|---|---|---|
  | `InvaderCore { level }` | `Dismantle { room, pos }` (the core's tile) | `siege_quad` | `MEDIUM` | Exercises formation travel + threat orientation + breach targeting (a stronghold core is shielded by ramparts). Affordability gate (`invader_core_attack_score`) preserved. One standardized `siege_quad` for all levels; level-aware sizing is a future refinement. |
  | `AttackFlag` (operator) | `Secure { room }` | `quad_ranged` | `HIGH` | Explicit operator intent — clear the flagged room. |
  | `ResourceDenial` | `Harass { room }` | `solo_harasser` | `LOW` | Deny a hostile player's remote income; opportunistic, never starves core-clearing/defense. (ADR 0029 §7/D7 later routes the *defended*-room case through a sized+gated raid instead.) |
  | `InvaderCreeps` | — (RECONCILED away) | — | — | Dropped: the reserved-remote-invader `Defend` context (`run_defense_scan`) already fields a squad on the identical trigger (`reservation().mine() && visible() && hostile invaders`), so a `Secure` here would double-field the same room. Defense owns it. |
  | `ProactiveDefense` / `ThreatResponse` / `Expansion` | (`Secure`/`Defend` when produced) | — | — | Enum variants the offense scan does not produce → nothing to migrate. |
  | `PowerBank { power }` | `Farm { PowerBank, room }` | `power_bank_duo` + bespoke dropped-power haul | `LOW` | **Deferred** — needs the highway dropped-power collector, which exists only inside the legacy `AttackMission::Exploiting` state, and warrants its own ADR (healed squad + predictive collector). |
  | **Heavy multi-squad player assault** (`plan_by_detected_threat`, towers≥4 → drain-duo + quad with `DeployCondition` sequencing) | — | — | — | **Deferred** — does NOT fit one-squad-per-objective. The legacy path cannot be deleted until a multi-squad / sequenced-objective mechanism exists. |

  **Producer routing + cap.** All migrated reasons flow through one `source → (ObjectiveKind, priority, composition)` branch in the offense launch loop, so the dedup (highest-scored candidate per room) and one combined cap apply uniformly. A migrated room carries no `active_attacks` entry, so the per-scan re-evaluation re-asserts its objective idempotently (refreshing the TTL). **War-side cap:** the launch-loop budget is `active_attacks.len() + (Attack-owned objectives in the queue)`, capped at `max_concurrent_attacks` — an existing objective is always re-asserted (only *new* offense is gated, score-sorted so the skipped one is the lowest-value); this replaces an `active_attacks`-only break. The manager's `MAX_CONCURRENT_SQUADS` is the downstream fielding limit.
- `WarOperation::run_defense_scan` → `Defend` (replaces `SquadDefenseMission` creation; **keep** the `hostile_warrants_defender` body-parts trigger and the owned-room-invader rule).
...
1. **Claim / retask.** Select `best_unclaimed` (priority then proximity to candidate home rooms; skip `unwinnable`-backoff and already-claimed), then `claim(objective_id, squad_id)`. Claims live **only in the ephemeral runtime map**; a `release_dead`-equivalent frees claims whose squad no longer exists each tick.
2. **Composition → roster.** Create the `SquadContext` entity and point each member's marker-converted `squad_entity` at it (ADR 0001). **Size members via the threat-matched `military/damage.rs` helpers + `bodies.rs sized_*_body`** — not fixed `BodyType` templates at `energy_capacity`. Preserve minimum-affordability, focus-heal (aggregate `enemy_focus_heal`), and energy-readiness (`SpawnNow`-vs-`Wait`).
3. **Spawn-demand generation (the only combat spawn producer).** Emit desired-minus-current as `SpawnDemand`s (ADR 0011 D1) tagged with one `GroupId` per squad for synchronized align-finish (§6). Replaces both missions' hand-rolled token broadcasts.
4. **Pre-spawn replacement, never renew** (ADR 0011 D4/D8). For any member with `ticksToLive < spawn_time + travel + PRESPAWN_MARGIN`, emit a successor demand for that slot. **No combat `request_renew` call sites.** If a fresh-TTL successor would badly mismatch low-TTL survivors, prefer retire-and-rebuild the whole squad (the generalized `handle_wave_wipe`).
5. **Compute per-tick tactical orders** (§4) — the brain that was smeared across `AttackMission::Engaging::tick` lives here so defense and offense share one model. The manager drives the squad **anchor** for cohesive formation travel (`advance_squad_virtual_position`), **formation orientation** toward the threat for box-fighting squads (pure `decide_squad.orientation` → `reassign_slots`), and **breach-aware dismantle targeting** (`breach_redirect` over the rover `room_grid_dijkstra` — break the rampart shielding the target first). Combat style (box-siege vs kite) is derived from the objective kind (`Dismantle` → Formation, else Skirmish).
...
The manager computes, per squad, one authoritative order set into `SquadContext.members[*].tick_orders`. Jobs **execute, never re-decide** the *combat target/heal/kite-vs-engage decision*. **(⚑ But the actual *movement request* to the pathfinder belongs in the squad/job, not the mission/manager — see the flagged tension in §5; the manager supplies the goal/anchor/focus as context, the job issues the move.)** All combat intents route through the **one guarded sink** (ADR 0003 §A.2 / IBEX-029) — the pre-overhaul path fires bare `creep.attack(...)`; this closes it.

**§4.1 — The tactics are PURE and live in `screeps-combat-decision`, so they are simulated, not just live (operator 2026-06-18; ADR 0006 §B.2).** The decision functions are pure over JS-free DTOs and run **identically** in the live bot and the in-process micro-sim — no tactics fork, self-play-validated, and iterated via the [ADR 0008a](0008a-combat-tactics.md) EXP-* register on the sim. The layering is two pure tiers + thin game-coupled adapters, with **clear responsibilities**:

| Layer | Where | Responsibility | Purity |
|---|---|---|---|
| **Per-squad decision** `decide_squad(SquadView) → SquadDecision` | `screeps-combat-decision` | shared focus (`select_focus_target`), engage/retreat **coupled hysteresis**, heal **assignment**, slot/orientation | **PURE** |
| **Per-creep decision** `decide_combat` / `decide_movement` | `screeps-combat-decision` | one creep's attack/heal intents + its movement *goal* (kite/engage/flee) given the squad orders | **PURE** |
...
| **SquadContext** | `military/squad.rs` (component) | DATA carrier: roster, cached member status, layout, the per-tick orders/state. Its `compute_heal_assignments`/`should_retreat`/`reassign_slots` **belong in `decide_squad`** (they are already pure over member data). | data |

So the SK duo and the defense squads run `decide_squad` — the SAME code a sim scenario exercises — and the manager carries zero tactics logic. **The pure surface** is `decide_squad`/`decide_squad_with_pathing` (shared focus + engage/retreat coupled hysteresis + heal assignment + the cohesive **pathfinding-scored kite goal**) feeding the per-creep `decide_combat`/`decide_movement`; the kite/flee search is rover's `LocalPathfinder::search_scored` (combat supplies only the pricing). Cohesion is a *term in the tile score* — the block moves to one shared goal — and the only individual break is a critical-HP flee.

**Two known limits (L1/L2; operator 2026-06-18):** **(L1)** the scored kite/flee search is **single-room** (`LocalPathfinder`), so a squad **will not flee to an adjacent room** — fine for an intra-room fight (cross-room travel is the separate phase) but a real edge for a squad cornered at a room boundary; the likely answer is a server-`PathFinder` multi-room-flee fallback when the local search is cornered (live-only, not sim-validated). **(L2)** the per-tick **eager DTO copy** (`creep_to_dto`/`structure_to_dto`) on the live path could be replaced by the **trait-based lazy-view** pattern the pathfinder already uses (`CreepHandle`/`CostMatrixDataSource`) — a `CombatCreep`-style trait the live adapter impls over `game::*` and the sim over `CombatWorld`, decisions generic over it — to drop the live copy/alloc. Gate that on a measured CPU win, since value-over-DTO keeps the digest + tests simple.

- **Authoritative focus-fire with ranked fallback.** Pick one focus target per squad from the **whole-squad centroid** (fixing the bug where the anchor's "first living member's room" returns `None` for everyone). A creep *in range of the focus target MUST hit it* (concentration is required to out-DPS the aggregate enemy heal — the engine nets damage-then-heal per target); a creep *out of range* uses a **manager-supplied ranked fallback list** (one shared scan), never its own re-scan. This both restores concentration and removes the 4× redundant per-creep room scans.
...
**The movement model is ANCHOR-PRIMARY with a column-collapse fallback** (corrected 2026-06-16 from an earlier "delete the anchor, use lead-follower" draft — see ADR 0003's *Movement-model analysis*, the rationale ADR 0003 retains while keeping only the guarded-sink and transient-tolerance invariants for itself and handing the mover here; lead-follower has a *fixed* offset with no facing, so it structurally cannot do orientation/rotation/present-armor/turn-from-tower, which are the tactically decisive maneuvers). This is the single biggest lever for "squads scatter." The pre-overhaul virtual-anchor is broken not because anchors are bad but because it advances in a **straight line** (no pathfind) while members path independently and the mode ratchets into Loose — all fixable. The bot already contains ~90% of the proper anchor machinery, dormant.

> **⚑ KNOWN ARCHITECTURE TENSION — the mission/squad/job movement split.** The split this ADR draws — the `SquadManager` computes per-tick movement orders (`TickMovement` `Engage`/`Kite`/`Hold` in §4, the anchor advance) and the job *"executes, never re-decides"*, with mission-side flee (`issue_virtual_anchor_flee`) and mission-side anchor stepping (`advance_virtual_pos`, `formation.rs`) — **inverts the bot's normal pattern**, where the creep's **job** state machine issues its own movement requests to the pathfinder (`jobs/utility/movebehavior.rs`; the economic jobs already do this). **The intended split:** missions/manager set **goals, objectives, and context** (the objective, the shared anchor `virtual_pos` + orientation, the focus target, the per-source suppression signal); the **squad or job performs the actual movement request to the pathfinder**. The shared *context* (anchor frame, cohesion gate, focus) is legitimately squad/manager-level — only the *movement-request issuance* should move down. Consistent with [ADR 0018](0018-source-keeper-room-exploitation.md) principle 8 and the phase-2 §2.1 convention.

- **Anchor as the squad's coordinate frame.** A `virtual_pos` + an **orientation**; each member's target = `anchor + rotate(base_offset, orientation)`. The anchor **follows a cached tile-path** — pathfound **once** through the pathfinding system, cached on `SquadPath` (the never-populated `room_route` is the vestigial intent), then followed step-by-step and re-pathed only on invalidation/stuck (the rover `CreepPathData.path` discipline). **Not** the straight-line `advance_virtual_pos`, which advances by a `signum` step with no pathfind (`formation.rs:380-414`) — the straight line is the bug, not the anchor. The cached path is built with a **footprint-aware cost transform** (the Overmind "moving-maximum" `applyMovingMaximum(w,h)` recipe — generalize the existing `apply_quad_cost_overlay`/`apply_formation_cost_overlay`, parameterized by W×H so duo/quad/larger share one path) plus `apply_tower_avoidance_costs` pricing — so the block never routes where it can't fit and naturally drifts out of tower-optimal range. Members then **move in lockstep** (one direction from the anchor's next cached step) — one (cached) pathfind + N cheap direction-moves, cheaper than N per-member `move_to`. Honors *pathfinding lives in the pathfinding system; modules supply pricing only*.
...
- **Boosts:** wire `composition.required_boosts()` → boost reservation at schedule time (ADR 0011 D9 / 0010), gate deploy on `is_ready`, behind the existing kill-switch (IBEX-027 wire-or-delete → **wire**).

## Alternatives Considered

| Option | Pros | Cons |
|---|---|---|
| **Scout-style `CombatObjectiveQueue` + `SquadManager` + tactics model** (chosen) | Matches the operator's reference exactly; work is pulled, so a dead producer never strands a squad; assignment ephemeral (no dangling refs); one cohesive model for offense+defense; tactics finally exist; everything measurable via ADR 0006 | New queue + manager; gated on ADR 0001 (marker-converted squad reference) + ADR 0003 (cohesion invariants) + ADR 0006 (harness); a labelled migration |
| Direct `register_objective`/`objective_status` mission↔manager API (the 2026-06-09 framing) | Slightly less machinery | Keeps the *mission* as work-owner → re-creates the push coupling that orphans creeps; diverges from the scout reference |
...
| Movement: keep the straight-line virtual-anchor | No change | The actual bug source — anchor doesn't pathfind, members path independently, mode ratchets into Loose |
...
**Positive**
- **The operator's three symptoms close structurally.** Orphan/idle → queue ownership + `Recall`; scatter → the anchor mover + hard cohesion + synchronized spawn; null tactics → the manager-computed focus-fire/kite/heal/hysteresis orders.
...
- **Hard gating:** unsafe before ADR 0001's marker-converted squad reference and incomplete without ADR 0003 (the cohesion invariants + movement-model analysis) and ADR 0006 (the harness that proves each step). It therefore slots **after** the harness-first work.
- **Pre-spawn mis-estimation / cohesion stall** — bounded by the cohesion/deadline force-abort and the give-up backoff.
...
**Sequencing is harness-first** (operator decision, 2026-06-16). ADR 0006's combat micro-sim + cohesion metrics + military score term land **before** the behavior steps below, so every step is measurable and replay-diffable. The behavior steps are then gated behind ADR 0001 (the marker-converted squad reference) and ADR 0003 (the cohesion invariants) per those ADRs' ordering. `WORLD_FORMAT_VERSION` in `game_loop.rs` MUST bump on any serialized-shape step.
...
| **0** | **Quick wins** (pre-manager): whole-squad-centroid focus target; recompute `heal_power` + boost-aware heal math; wire `check_movement_failure` (IBEX-015); add a `Recall` terminal state so orphaned creeps recover | `attack_mission.rs`, `squad.rs`, `squad_combat.rs` | Behavioral |
| **1** | ~~**Wire `SquadDefenseMission` onto `SquadContext`**~~ — **SUPERSEDED by Step 5**: `squad_defense.rs` is removed entirely rather than wired, and defense goes straight to the `Defend` objective. | ~~`squad_defense.rs`~~ | Moot |
| **2** | **Proper anchor mover** replaces the straight-line `advance_virtual_pos`: footprint-aware ("moving-maximum") anchor pathfind + lockstep block advance + hard cohesion gate; wire dormant orientation (`threat_direction`/`orient_toward`/`reassign_slots`/`mirror_y`); add the **column-collapse** single-file relaxation for corridors/edges + **loose-centroid** for N>4. Breach-aware dismantle targeting rides on the same mover (`room_grid_dijkstra` lives in `screeps-rover`), and threat orientation is projection-based (no layout double-rotation). | `formation.rs`, `squad.rs`, `squad_combat.rs`, pathfinding system, `screeps-rover` | Behavioral |
| **3** | **Marker-converted `squad_entity: EntityOption<Entity>`** (ADR 0001) replaces the bare `squad_entity: Option<u32>` | `jobs/squad_combat.rs:18`, `jobs/data.rs`, `cleanup.rs` | Memory/format (one loud reset) |
| **4** | **`CombatObjectiveQueue` + `SquadManager` + tactics; migrate `AttackMission`→producer; delete the legacy** (`AttackMission`/`AttackOperation`/`AttackReason`) | `military/objective_queue.rs` + `military/squad_manager.rs`; `war.rs` produces objectives; `attack_mission.rs`/`attack.rs` deleted | Memory/format + Behavioral |
| **5** | **Migrate defense + escort onto the queue**; `WarOperation` becomes a supervisor (withdraw/trim — IBEX-026/028); add `UnwinnableTarget` backoff | `war.rs`, `squad_defense.rs` removed, `claim.rs` escort producer | Behavioral |
| **6** | **Synchronized spawning** via `GroupId`/align-finish/pre-spawn; boost handoff (or kill-switch off) | `spawnsystem.rs` executor unchanged; ADR 0011 D3/D4/D9 | Behavioral (S2 boost handoff depends on the ADR 0010 lab pipeline) |
...
Explicitly outside this architecture, each for a structural reason rather than a scheduling one:

- **Power-bank farming** — the `Farm{PowerBank}` objective kind exists, but the healed squad + the predictive dropped-power collector need their own ADR; the collector logic currently lives only inside the legacy mission.
- **The heavy multi-squad player assault** — drain-duo + quad with `DeployCondition` sequencing does not fit one-squad-per-objective. It needs a multi-squad / sequenced-objective mechanism — several concurrently-named squads under one objective — before it can move off the legacy path.
- **Cross-room flee (L1)** and the **trait-based lazy combat view (L2)** — §4.1; the latter is gated on a measured CPU win.

### Pulls from the expansion lifecycle (ADR 0017)

ADR 0017 (threat-aware expansion) shipped the safe-claim / abort half but **assigned two squad-dependent pieces to this overhaul**:

1. **Expansion escort / pre-clear — a new objective kind.** When a claim target is *marginal* (a transient/weak threat, economically worth taking), the claim pipeline declares an `Escort{room}` objective — pre-clear (the salvage `DismantleJob` for a remnant spawn/tower; a small squad for a weak combat creep) and hold it clear while the `[Claim,Move]` claimer commits. `DefenseEscalation::from_threat` is already `pub` for sizing it. Without the escort, ADR 0017 conservatively treats marginal rooms as unsafe (reject, never escort) — that is the designed fallback, not a gap.
2. **The defense-staleness retirement is ownership-subordinate.** ADR 0017 §13 requires a defense to self-terminate the moment its room stops being `owner().mine()`. Step 5 preserves that invariant on the queue: a `Defend` objective for a room we no longer own is **withdrawn immediately** (its `SuccessPredicate` encodes this), not merely when "threat clears + members dead." This is also the teardown cascade for ADR 0017's `unclaim()` abort.
...
1. **The defense trigger must key on threat by body parts, not just combat parts.** `RoomDynamicVisibilityData::hostile_creeps()` flags only ATTACK/RANGED_ATTACK/WORK, so an enemy CLAIM creep neutralising the controller (or a lone dismantler/healer) was invisible and no defense was created. The trigger must also fire on `hostile_threat_creeps()` and commit a defender to hostiles bearing a "worth-defending" part (Attack/RangedAttack/Work/Claim/Heal) — `war.rs` `hostile_warrants_defender`. The manager's `Defend` trigger inherits this: a controller-attacker is a defendable threat.

2. **A body's MINIMUM buildable cost must be affordable at the lowest RCL the role serves.** `solo_defender_body`'s repeat unit `[RangedAttack, Move, Heal, Move]` (500e) made `create_body` return `Err` below 500e, so a young RCL2 room spawned nothing and was declaimed undefended. **Rule: never force an expensive part (HEAL/TOUGH) into the minimum repeat — size it as a fixed, affordability-gated addition.** The same latent bug existed in `duo_healer_body`.

3. **Threat-matched sizing, built as a direct `Vec<Part>`.** Pure helpers in `military/damage.rs` (`attack_parts_to_kill`, `defender_heal_parts_for_dps`, `defender_spawn_readiness`) turn `RoomThreatData` + room energy into a body; the sized builders (`bodies.rs sized_defender_body`/`sized_healer_body`) construct the final `Vec<Part>` directly (offense → HEAL when affordable → TOUGH when affordable → MOVE, degrading to a `[RangedAttack, Move]` 200e floor), bypassing the `&'static [Part]` repeat-template constraint. The manager's composition→roster step sizes members this way, not from fixed `BodyType` templates at `energy_capacity`.
...
6. **Multi-room sourcing via one shared spawn token.** A defender is sourced by broadcasting ONE `spawn_queue.token()` to every in-range home room (≤ `MAX_DEFENSE_SOURCE_DISTANCE`); the shared `spawned_tokens` set fulfils it at most once. The manager's reconciler uses this pattern for shortfall demand: one token per slot, several candidate rooms.
...
The sizing helpers (`damage.rs`) and the `sized_*_body` builders fold into the manager's composition→roster + pre-spawn steps; the minimum-affordability (§2), focus-heal (§4) and energy-readiness (§5) properties above must not regress in the process.
