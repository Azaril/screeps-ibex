...
| **AttackMission** (`attack_mission.rs`) | ✅ `attack.rs` (per wave) | ✅ `SquadContext` + virtual anchor | **MERGE — IN PROGRESS (P2.G4-offense, sequenced).** Becomes an objective *producer* (`Secure`/`Dismantle`/`Harass`/`Farm{PowerBank}`); lifecycle + tactics move to the `SquadManager`. Manager siege capabilities built (O1 anchor-mover / O2 orient / O3 breach / O4 wave-retry). **First producer live: `InvaderCore`→`Dismantle` (O6.1, 2026-06-18), coexisting with `AttackOperation` for the remaining reasons.** Remaining: O6.2 (rest of the single-squad reasons) → O5 (power-bank) → O7 delete. See [`../execution/g4-offense-plan.md`](../execution/g4-offense-plan.md). NOT yet removed. |
...
- `WarOperation::run_offense_evaluation` → `Secure`/`Dismantle`/`Harass`/`Farm` (player offense gated by ADR 0014 `WarDecl`; NPC policing autonomous). **The offense-producer mapping (design):** each live `AttackReason` maps to one objective kind + a threat-/role-matched composition + a priority, mirroring `run_defense_scan`'s escalation→composition pattern:
  ...
  |---|---|---|---|---|
  | `InvaderCore { level }` | `Dismantle { room, pos }` (the core's tile) | `siege_quad` | `MEDIUM` | **DONE (P2.G4-O6.1).** Exercises O1 travel + O2 orient + O3 breach (a stronghold core is shielded by ramparts). Affordability gate (`invader_core_attack_score`) preserved. Standardized `siege_quad` for all levels (level-aware sizing is a future refinement). |
  | `InvaderCreeps` | `Secure { room }` | `solo_ranged`/`duo` | `MEDIUM` | O6.2. **Reconcile with the migrated remote-defense `Defend`** — don't double-field the same room. |
  | `Flag` (attack flag) | `Secure { room }` | escalation-sized | `HIGH` | O6.2 (operator-directed). |
  | `ResourceDenial` | `Harass { room }` | `solo_harasser` | `LOW`/`MEDIUM` | O6.2 — deny, don't hold. |
  | `ProactiveDefense` | `Secure`/`Defend` | escalation-sized | `MEDIUM` | O6.2. |
  ...
  **Coexistence (current state):** during the migration the offense producer upserts objectives for the *migrated* reasons and keeps launching `AttackOperation`s for the rest, so the two paths run side by side until O7. A migrated reason no longer pushes an `AttackCandidate`/enters the `AttackOperation` launch loop, and the room carries no `active_attacks` entry, so the per-scan re-evaluation re-asserts the objective idempotently (refreshing its TTL). **War-side cap:** O6.1 relies on the manager's `MAX_CONCURRENT_SQUADS`; a dedicated active-offense-objective count (replacing `count_power_bank_attacks`/`active_attacks`) reconciles the war cap as O6.2 lands.
...
| **1** | **Wire `SquadDefenseMission` onto `SquadContext`** (ADR 0003 §B.1 dominant fix) | `squad_defense.rs` → `new_with_squad` | Behavioral |
...
| **4** | **`CombatObjectiveQueue` + `SquadManager`** behind the live offense path (parity), then migrate `AttackMission` to a producer; manager computes tactics; generalize `handle_wave_wipe`; delete combat `request_renew` | new `military/objective_queue.rs` + `military/squad_manager.rs`; `attack_mission.rs` shrinks; `war.rs` produces objectives | Memory/format + Behavioral — **queue+manager+tactics DONE (G1/G2/G3/G3-tail + G4-O1/O2/O3 + O4 wave-retry/unwinnable); migrate `AttackMission`→producers IN PROGRESS — first producer live: `InvaderCore`→`Dismantle` (O6.1), coexisting with `AttackOperation`; remaining single-squad reasons = O6.2, power-bank `Farm` = O5, delete = O7 (soak-gated). Full offense checklist: [`../execution/g4-offense-plan.md`](../execution/g4-offense-plan.md).** |
