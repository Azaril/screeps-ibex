...
| **AttackMission** (`attack_mission.rs`) | ✅ `attack.rs` (per wave) | ✅ `SquadContext` + virtual anchor | **MERGE** — becomes an objective *producer* (`Secure`/`Dismantle`/`Harass`); lifecycle + tactics move to the `SquadManager`. |
| **SquadDefenseMission** (`squad_defense.rs`) | ✅ `war.rs` defense scan | ⚠️ **squad-LESS** (`squad_entity=None`) — scatter by construction | **MERGE** — becomes a `Defend` objective on the same manager. Defense and offense converge on one cohesive, focus-firing model. (ADR 0003 §B.1 "wire defense onto `SquadContext`" is the first quick-win step.) |
...
- `WarOperation::run_offense_evaluation` → `Secure`/`Harass`/`Farm` (player offense gated by ADR 0014 `WarDecl`; NPC policing autonomous).
...
5. **Compute per-tick tactical orders** (§4) — the brain that was smeared across `AttackMission::Engaging::tick` moves here so defense and offense share one model.
6. **Enforce invariants & retire** (§5).
...
The manager computes, per squad, one authoritative order set into `SquadContext.members[*].tick_orders`. Jobs **execute, never re-decide**. All combat intents route through the **one guarded sink** (ADR 0003 §A.2 / IBEX-029) — today combat fires bare `creep.attack(...)`; this closes it.
