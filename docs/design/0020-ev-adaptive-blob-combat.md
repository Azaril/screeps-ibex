...
**Next: R6** (SK keeper sizing — give the SK duo a keeper `DefenseProfile` + size RangedDPS, so the live SK farm holds). R6 needs `RequiredForce` to gain `ranged_parts` (today it carries heal/dismantle/tough; `sized_for` sizes Healer/Dismantler/Tank, not RangedDPS), and note the SK suppression model is positional (mine when the keeper is away), not out-heal-continuously — so R6 sizes the duo to *survive a keeper engagement*, not to tank keepers 24/7. Then the soak (P3).
