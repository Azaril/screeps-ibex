...
**Next: R4** (P(win) model — `assess_engage` balance → probability; replaces the fixed 1.3 with a P(win) target), then **R5** (importance·P(win) investment — scale the margin by objective value), then **R6** (SK keeper sizing — give the SK duo a keeper `DefenseProfile` + size RangedDPS, so the live SK farm holds). Note R6 needs `RequiredForce` to gain `ranged_parts` (today it carries heal/dismantle/tough; `sized_for` sizes Healer/Dismantler/Tank roles, not RangedDPS).
