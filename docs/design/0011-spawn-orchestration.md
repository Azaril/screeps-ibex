...
**Decision (Q10, source of truth [docs/reviews/adr-closeout-2026-07-02.md](../reviews/adr-closeout-2026-07-02.md) §2).** Re-scope this ADR to its **economic half** and its **Step-0 executor quick-wins**; **drop** the combat-cohesion pieces.

- **BUILD (economic orchestrator):** D2 (throughput × energy budget model) → D5 (cross-room assist / the G3 incubation seam) → D7 (starvation cure: refill-stall demand re-sizing + always-affordable `Bootstrap` class). This is the "convert an economic lead into territory" capability the competitive verdict demands, and the RCL8 spawn-budget argument stands independently.
- **DO FIRST (Step-0 executor quick-wins, None-breaking):** the three verified-real, non-behavior-breaking executor fixes — (a) engine-true renew energy decrement `ceil(1.2 × body_cost / 3 / body.len())` replacing the `(body_cost * 2) / 5` over-charge (`spawnsystem.rs` renew pass); (b) move the renew pass **behind** the priority check so a renew-eligible creep never consumes a lane a CRITICAL/HIGH request wants this tick (fixes the P4 renew-before-CRITICAL inversion); (c) `debug_assert!(priority.is_finite())` at the request seam (IBEX-046) + the descending-order comparator unit test. **Do NOT reverse the `spawnsystem.rs` priority comparator — it is verified-CORRECT** (the review false-positive guard; reversing it is the actual hazard).
- **DROP (obviated combat-cohesion pieces):** D3 align-finish group admission, the D4 **combat** pre-spawn arm, and the D8 **combat** renew confinement. **Why:** the trickle-spawn / staggered-quad cohesion pathology (P3) and the combat renew-as-glue hang (P4/Field Report B) these were designed to fix are **already solved** by the shipped **objective-queue / auction lifecycle** (ADR 0027/0032), deployed live at **WFV 26** — the squad-manager auction spawns and forms the squad, and the combat `request_renew` glue is gone. Building D3/D4-combat/D8-combat now would re-solve a solved problem against a superseded architecture.
- **NOTE — anchors have drifted.** The body's `spawnsystem.rs:NNN` / `attack_mission.rs:NNN` / `economy.rs:NNN` line-anchors were written 2026-06-09 and the files have since changed (combat overhaul, auction lifecycle). **Re-verify every cited anchor against current code before touching it** — the review verified the mechanisms (comparator-correct, renew over-charge, renew-before-CRITICAL) still exist, but not at the quoted line numbers.

### Resume-point
Do the Step-0 executor quick-wins first: in `screeps-ibex/src/spawnsystem.rs`, replace the renew energy decrement with the engine-true `ceil(1.2 × body_cost / 3 / body.len())`, then move the renew pass behind the priority check, then add the `is_finite` assert + descending-order comparator test. Ship as one None-breaking change; then start the economic orchestrator at D2 (the throughput × energy budget model over `RoomEconomyData`).

> **Update (2026-06-16) — confirmed by the combat overhaul (ADR 0008 revision).** The trickle-spawn pathology (P3: a quad's slots as 4 independent MEDIUM tokens fulfilled in `HashSet` order, members born rooms-apart and ticks-apart) is one of the operator's three named cohesion failures — members scatter *before the fight starts*. This ADR's D3 (align-finish group admission), D4 (pre-spawn), D8 (renew confinement), and D10 (MEDIUM→HIGH on commit) are adopted by ADR 0008 verbatim; the **`SquadManager` is the sole combat demand producer** (`GroupId = SquadId`). Sequencing note: ADR 0008's behavior steps are now **harness-first** (ADR 0006 combat micro-sim lands before them), so the group-spawn/pre-spawn steps here are validated by the deterministic cohesion metric, not only the private-server gate.
