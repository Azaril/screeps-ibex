...
> **Split of ownership with the ADRs that overtook parts of this design.**
> - **The arbitration currency is ADR [0040](0040-stress-energy-prioritization-and-economy-sim.md)'s unified e/t market, not a bespoke budget.** D2's throughput × energy model and D7's starvation cure state *what the budget must express* — part-ticks of lane capacity, committed load, backlog pressure, refill trend, and the anti-starvation sizing rule. The *arbitration* is 0040's: spawn demands are numeric e/t bids against an opportunity floor, and deficit-priced spawn bids are the starvation cure. This ADR must not grow a second, parallel budget.
> - **Combat group cohesion is owned by the objective-queue / auction lifecycle (ADRs [0027](0027-objective-squad-lifecycle.md)/[0032](0032-ev-optimal-squad-assignment.md)).** The staggered-quad pathology (P3) and the combat renew-as-glue hang (P4 / Field Report B) are solved there: the squad-manager auction spawns and forms the squad, and combat `request_renew` glue does not exist in that architecture. D3's align-finish admission, D4's **combat** pre-spawn arm, and D8's **combat** renew confinement are therefore recorded here as design, but are **not** this ADR's to build — what remains here is the economic arm of each.
> - **What this ADR still owns end to end:** D1 (the declarative demand seam), D5 (cross-room assist, including the G3 incubation seam, consuming 0040's e/t currency), D6, D9, D10, the economic arms of D4/D8, and the Step-0 executor corrections.
>
> **Line anchors are navigational, not contractual.** The `spawnsystem.rs:NNN` / `attack_mission.rs:NNN` / `economy.rs:NNN` anchors below point at *mechanisms* (the descending comparator, the renew over-charge, renew-before-priority, the demand-production sites); the mechanisms are what the design reasons about — re-locate them in current code rather than trusting the numbers. In particular: **the `spawnsystem.rs` priority comparator is verified-CORRECT — adapt it to bid keys, never reverse it.** Reversing it is the actual hazard.
...
*(These are the quantities the budget must expose; the arbitration over them is ADR 0040's e/t market — bids against an opportunity floor — not a second budget mechanism here.)*
...
*(Combat groups are formed by the ADR 0027/0032 auction lifecycle; this section is the general statement of the mechanism, and applies here only to any non-combat group demand.)*
...
*(The economic arm — seamless miner/hauler handover — is this ADR's; the combat arm is the auction lifecycle's, ADR 0027/0032.)*
...
*(The pricing form of this cure — deficit-priced spawn bids — belongs to ADR 0040's market; the reasoning below is why reservation is kept and why the cure must live on the demand side.)*
...
3. **Step 2 — Group spawning + pre-spawn. Breaking: Behavioral.**
   Enable `GroupId` align-finish scheduling and `prespawn` on demands. The combat arm of this step belongs to the ADR 0027/0032 auction lifecycle, which forms squads itself; what lands here is the economic arm. **Validate:** emergence-tick spread within W for any group demand; kill an incumbent mid-life — successor emerges before incumbent death − margin; zero renew intents outside D8's predicate.
