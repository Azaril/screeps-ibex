...
- **Related:** ADR 0040 (the e/t currency, `value_e`, §D5.4 `R_O`, the spawn-request policy), ADR 0038 (`room_net_roi` / reservable-remote economic pricing), ADR 0031 (capability force composition + `pairing_p_win`), ADR 0037 (count-quorum winnability veto), ADR 0028/0034 (the lifecycle/forming harness); this model supersedes the interim band-scaled `forming_completion_bid(present, ticks)` escalation in ADR 0040's spawn lane; memory [[combat-ev-economic-and-pwin-gating]], [[prefer-per-tick-optimal-over-hysteresis]], [[sim-determinism-fence]].
...
- **The floor is `0` until the civilian spawn lane is true-EV.** `opportunity_floor = 0` is the sound value-negative bound (abandon a squad whose completed rate cannot cover its burn); raising it above zero — the genuine economy-alternative comparison, "abandon value-positive-but-economy-is-better" — is only commensurable once the civilian spawn lane bids true-EV `body_roi` (ADR 0043 A2). Until then the floor term is present in the formula and pinned at 0.
...
**P0 (deploy-safe, WFV-unbumped — all ephemeral, recomputed per tick, nothing serialized):** `p_win_completed` + `R_O_completed` + the reserved-band bid + the economic give-up (with EMA + K-latch) + the safe-mode arm; delete `FORMING_WASTE_STEP_MILLI`; re-pin fixtures; grep-audit; matrix + fence.

**Additive refinements (non-blocking, each closes a named gap):**
...
- **Escalated-out stragglers are bounded by the give-up, not recycled:** a member dropped from the gather quorum (stalled past `SOLO_TRAVEL_STALL_WINDOW`) stays rostered and can camp at home until the squad's economic give-up retires it — the give-up is what bounds the waste. Recalling an escalated-out member to recycle was considered and rejected: against a persistent block it trades a camping creep for re-spawn churn, the war-lifecycle whack-a-mole failure mode.
