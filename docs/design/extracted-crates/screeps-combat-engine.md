# screeps-combat-engine — preserved internal notes

> Snapshot of the pre-cleanup `README.md` for `screeps-combat-engine` (the internal
> version with ADR/phase-code references + in-progress status), preserved when the
> README was rewritten for external readers. The crate lives at
> https://github.com/Azaril/screeps-combat-engine .
