...
- **ADR 0026 IMPLEMENTED (2026-06-26):** the objective/information-dependent strategy-selection layer (`open_combat()`=`a1-i6-tight`, `breach()`=`a1-i4-def`, safe-mode veto) shipped through the decision crate + the bot seam. **Harness caveat surfaced during adoption:** base-attack absolute scoring is **noise-dominated (~1% cross-process** — the sim's `HashMap` iteration), so it cannot robustly separate low-approach profiles; only symmetric open-combat self-play reproduces. The objective split rests on the robust open-combat win + the dismantle-needs-range-1 principle. A **deterministic-sim-map** fix is the follow-on that would make base-attack a reliable tuning signal.
