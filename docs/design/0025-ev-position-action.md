...
- **Adoption:** no single `KernelParams` wins both lenses → the principled fix is an **objective-aware approach coefficient** (weight `approach_coef` up when the objective is a STRUCTURE/base, default for open-creep combat) — requires the kernel objective wiring (§11 #11). Recommended adoption direction; the shipped default is unchanged (robust + the breach gap is closed by objective-awareness, not a global bump that would forfeit open-combat robustness). **Re-tune machinery is clean + reusable for the joint "tune the complete system" pass.**
