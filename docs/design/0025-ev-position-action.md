...
- **ADR 0026 IMPLEMENTED (2026-06-26):** the objective/information-dependent strategy-selection layer (`open_combat()`=`a1-i6-tight`, `breach()`=`a1-i4-def`, safe-mode veto) shipped through the decision crate + the bot seam. **Harness caveat surfaced during adoption (now RESOLVED):** base-attack absolute scoring was noise-dominated (~1% cross-process), so it could not robustly separate low-approach profiles; only symmetric open-combat self-play reproduced. **The determinism follow-on LANDED (2026-06-26):** two seed-ordered hash iterations in `screeps-rover`'s resolver were the source — (1) the per-tick pathfinding-ops budget consumed in `topological_sort_follows`'s seed order on dense bases, and (2) `current_pos_to_entity`'s last-write-wins when two creeps stack on a tile (a transient cross-room border stack) — both fixed (sort the topological move order by `Handle`; keep the lowest `Handle` on a position collision). The sim is now **bit-deterministic** (`sim_is_deterministic_over_rounds` asserts spread 0 across 5 fresh-seed rounds), so base-attack is a reliable tuning signal and a clean re-tune is now possible. The objective split still rests on the robust open-combat win + the dismantle-needs-range-1 principle (the profiles were not re-derived).
- **SPACING re-tune (2026-06-26) — `open_combat` updated `a1-i6-tight` → `a1-i6-tight-s2` (spacing 1→2):** with the sim bit-deterministic, a per-situation discovery sweep (ADR 0026a) found the original 48-config grid had a **structural blind spot — it fixed `spacing=1`**. Screeps AoE is pure Chebyshev (no LOS), so a tight blob eats stacked ranged-mass-attack + overlapping tower fire. Once spacing is in the field, the old spacing-1 profile is negative-mean + exploitable (176); **spacing 2 is the best open profile over the real-opponent field** (`final_open_validation`: +169 mean, beats spacing-1 +135 at equal exploit 85≈84). Spacing 2 is the generic sweet spot; spacing 4 only wins a pure-ranged mirror (a candidate situational mode). The hand-designed ADR-0026a catalog modes mostly UNDER-performed the data sweep (e.g. `ranged_duel_kite`'s lower-incumbency idea LOST the ranged mirror) — **tournament-discovery > ideation here**, and the headline lever was the one the grid excluded. The melee "close-hard" signal is sustain-conditional (deferred); breach-side + asymmetric situational modes are deferred pending new beds/signals.
