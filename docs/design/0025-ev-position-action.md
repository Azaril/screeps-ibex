...
Stage 3 **LANDED** (eval `<pending>`) — `harness/foreman_capture.rs` (the reusable capture lib: `CapturedBase`/`CapturedStructure` + `capture()` calling `screeps_foreman::planner::plan_room` — the planner is a clean LIBRARY, NOT the bench binary, so no binary dependency) + the `capture_base` bin (offline, `CARGO_MANIFEST_DIR`-anchored, incremental) → committed `resources/captured-bases.json` cache + `ForemanGenerator` (loads cache, never plans) with `realize_base` + adaptive `breach_from_ramparts` (breach point = the real rampart ring's tile nearest a navigable entry). `decode_fast`/`fast_to_combat` bridge added. Captured real foreman bases (3 spawns / 6 towers / ~20-31 ramparts / ~20 walls each, ~5s/room). **Next = Stage 4** (re-tune over the full realistic basket: extend `Bed::Imported`, point base-attack at `ForemanGenerator`+`ImportedRoom`, adopt a `KernelParams` winner + re-validate).
