...
8. **Tournament basket enrichment BEFORE adopting any tuning lead (roadmap 0020-S4-RES; operator-recalled).** The current basket is 3 symmetric all-ranged beds — overfit-prone (it's why the k-tight-coh +286 lead and the older 0020-CAUTIOUS lead are NOT auto-adopted). Enrich with **asymmetric attacker-vs-defender objective beds** (reuse the harness `Designed`/`Permutations`/`ForceSpec` generators: terrain, ramparts, towers, breach, multi-room), the **§8.6 objective-aware turtle/stalemate scorer**, and **scripted-archetype-vs-managed** matches; *then* run the `KernelParams` tuning pass and adopt a winner deliberately. (PFSP opponent mixing + behavioral de-dup + formal Elo are the follow-on.)
