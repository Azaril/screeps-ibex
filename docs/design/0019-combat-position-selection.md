...
- **Status:** Accepted; **COMPLETE — Stages 0–4 on master (2026-06-19 through 2026-06-23)**; the **§8 healer heal-coverage increment landed 2026-06-23**; **§9 is the live-soak resume ledger.** The unified position utility is live in `decide_squad_with_pathing` as the default (no kill-switch). Stage 4's weight-sweep / EXP-* tuning loop is **shipped as a standing regression guard, not "ongoing"** — the deferred boosted-TOUGH conversion + weight-discriminating bed are opportunistic follow-ups, not open stage scope. The headline status line per-stage is the **(latest first)** log below.
- **Current state (as of 2026-07-01):** ADR-0019 is fully as-built and **deployed to MMO**. The surrounding combat program moved on: rally/engage robustness (ADR 0034), scout-before-commit/abandon-on-contact (ADR 0035), opportunistic structure targeting (ADR 0036), and tower-aware neighbour defense (ADR 0037) all **landed + deployed**, on top of the objective-squad lifecycle (ADR 0027), capability-driven force composition (ADR 0031/0031a/0031b) and EV-optimal squad assignment (ADR 0032). This ADR's position-utility is the substrate those layers reuse; nothing here is open work. §9's "watch a winnable clear" soak was superseded by that live combat program (squads now reach + engage + defend live).
- **Implementation status:** Stages 0–3 DONE; Stage 4 = ongoing EXP-* tuning. (Detail, newest first:) Stage 0 + Stage 1 core + **Stage 2 reachability core DONE** (2026-06-19). **Stage 2 (`1670723` rover + `1838e1a`):** the operator's flood-fill landed as rover **`reachability_from`** (multi-source weighted Dijkstra — soonest any source reaches each tile, per-source cadence, one flood regardless of N) + folded into kite scoring as a **future-threat term** (the kiter prefers durable standoff, not just safe-this-tick; `KiteThreat.step_ticks` cadence from the body, mobile-chaser-only seeds per Guard 5, `w_future` weight, `None`→byte-identical). **Stage 2-tail behavior DONE (`9d8d1f6`):** cohesion→true **g-cost** (wall-aware) via a centroid `reachability_from` flood — a tile reachable only around a wall is correctly "far" (identical to Chebyshev in open terrain; +1 test). **Folded into Stage 3 (judgment, not blocking):** the integer `ThreatField` *precompute*, the `boost` field + TOUGH/heal net conversion, and the perf scratch-buffer reuse + double-matrix-dedup — each is structural/CPU, not behavior: the precompute is a slight CPU *regression* for the current single-block case (its win is multi-block + the unified utility), the net conversion needs the field to be actual-hits (Stage 3), and the perf work is premature while not deploying. They land *with* Stage 3's unified utility + CPU bench, where they're exercised and gated together. A full sim **EXP-COHESION** scenario (vs the unit test) also rides Stage 3 (needs the squad-sim harness).
...
- **Stage 3a DONE (`28d6540`):** `PositionLayers` + `::build` (use-agnostic) + `.fields()`; `plan_kite_anchor` builds once (byte-identical). **Stage 3b started:** the **CPU compound-worst-case bench** is built (`8121b81`, `screeps-combat-eval::bench`) — the default-on gate; baseline **~1121 µs/block-tick** (4 blocks × 25 ticks; open room, 6 towers, 10 chasers), surfacing that the floods + matrix churn dominate. **Flood-dedup DONE (`0b38863` + rover `f83b339`):** `search_scored` now threads the settled path-cost `g` into its cost fn, so cohesion reads the kite search's own `g` (= distance-from-centroid) instead of a separate centroid flood; `PositionLayers` keeps only the shared threat flood; byte-identical → **bench 1121 → 950 µs/block-tick (~15%, 3 floods → 2)**. **Flee↔stand unification DONE (`e7a7ad9`):** `score_tile` gained a focus-engagement REWARD (negative cost for a tile in weapon range of the focus) + `KiteScoreParams::engage()` (the same scorer reweighted to stand-and-fight). So *flee* (kite preset, safety dominates) and *stand/close* (engage preset, focus-damage dominates) emerge from ONE scorer purely by the objective weights — the ADR's thesis, demonstrated. `w_focus_dmg` defaults 0 ⇒ kite byte-identical. decision 50. **Live engage positioning DONE (`01823d6`):** `decide_squad_with_pathing` (the live `SquadManager` decision) gained an ENGAGE branch — a ranged squad engaged with a creep focus and not kiting runs the scored search with `engage()` weights → `Advance{range:0}` onto the EV-optimal tile (or Hold), instead of a naive straight-line Advance. Mutually exclusive with the kite branch (one scored search either way); flee vs stand differ only by the preset. Ranged + creep-focus scoped (melee/siege keep range-1 Advance; breach keeps the breach Advance). +1 test. decision 51. This reaches the live bot (the manager already calls this fn). **FINAL NORMALIZED UTILITY DONE (`992b191`):** `score_tile` is the complete unified utility — every term normalized to `[0, SCALE]` (weights = dimensionless ratios; flee/stand emerge by reweighting): TAKEN (the ThreatField's net hits per fragile-HP — **#2/#3**, so ranged threats + towers now shape positioning, which the `reach-0` proxy ignored), FUTURE, COHESION (wall-aware g), PROXIMITY (focus beyond `r*`), OPENNESS, EDGE (anti-corner scored-path), DMG reward. **Weapon-range `r*`** parameterized (3 ranged / 1 melee). `FUTURE_HORIZON` 5→3 so a kiter HOLDS `r*` (durable standoff) vs fleeing to the horizon. Must-fixes **#1 (integer terms/field) + #2 (fragile-θ) + #3 (normalized) now complete** (with #4 veto + #5 + #6a earlier). CPU bench ~868 µs/block-tick. **BUILD-ONCE-PER-ROOM DONE (`4185aef`):** `plan_kite_anchor` / `decide_squad_with_pathing` gained `shared: Option<&PositionLayers>`; `build_room_layers()` pub helper; the live `SquadManager` holds a per-tick `HashMap<RoomName, (LocalCostMatrix, PositionLayers)>` and builds the threat field + floods ONCE per target room (they depend only on the room's enemies, not on which squad asks), sharing across every squad engaging there — the layer architecture's perf payoff (`Some(layers)` reuses; `None` builds inline = unchanged single-squad path; bench + agent sim pass `None`). `build_target_matrix()` extracted so the layer build + the kite search share one matrix. All crates green (engine 40 / rover 31 / decision 55 / agent 35 / eval 14). **focus_damage RICHNESS DONE (`717dd2a`):** the DMG reward is now the *net hits the squad would actually land* on the focus — a convex blend of EFFECTIVENESS (net/max-output → pulls a melee block to range 1) and KILL-PRIORITY (net/focus_hits → commit to a near-dead focus), 0 when out-healed (→ safety disengages); `SquadMemberView` gains melee/ranged power, `FocusDamage` carries focus hits + nearby enemy heal. `None` ⇒ the flat in-`r*` reward (tests/sim byte-identical). **DEFAULT-ON GATE DONE (`93c2063`):** the unified utility is the live default (no kill-switch — there's no flag path); the CPU bench is the standing gate — a generous per-block-tick budget (5 ms, non-flaky across debug/release) for BOTH the per-squad and build-once-per-room paths, plus a behaviour-preserving check (identical plans). **ADVANCE-TO-DAMAGE LAYER + EXP-COHESION + LATCH DROPPED (`c4a5266`):** the full-sim EXP-COHESION (managed squad threads a wall-gap corridor, focus-fires, survives) surfaced that the engage search — a *bounded* flood — stalled short of a far focus. Replaced the would-be "approach vs position" branch with a **dominant proximity layer** (engage `w_prox` 0.4→1.5): proximity *is* the advance-to-damage layer and is 0 inside `r*`, so ONE search both marches to the focus and picks the engage tile (the veto still forbids a lethal march). Proximity distance stays **Chebyshev** (Screeps charges equal diagonal/cardinal cost, so true distance = Chebyshev); its flat square iso-range plateaus — which let the deterministic tie-break drift the squad to a ring corner / off a corridor mouth — are broken by a small **perpendicular-deviation tie-break** from the centroid→focus line (capped + low-weighted so it beelines along the real approach line without distorting the movement-correct cross-ring ordering; `5e0fc68` corrected an interim euclidean magnitude that mis-priced diagonals). **#6b goal-latching was REMOVED, not added:** disabling it left every kiting/cohesion scenario passing unchanged (no observed oscillation), it didn't save CPU (full search still runs each tick), and it caused a bug (its stickiness pinned the advance short) — the deterministic tie-break (#6a) is the real, kept oscillation guard. **Stage 3b COMPLETE.**
- **Stage 4 SHIPPED (measure-first tuning loop, 2026-06-19):** the tunable seam + managed-squad EXP scenarios + weight-sweep regression guard are all landed on master — this is a **standing guard, not open work**. **tunable seam (`08c2331`)** — `decide_squad_with_pathing` gained a `SquadTacticParams { kite, engage }` param (`Default` = the shipped presets) so the EXP loop can sweep weights without touching the live presets (live + tests pass `Default`; `ManagedSimSquad::with_tactics` injects for the sweep). **managed-squad EXP scenarios (`f3b8ca2`)** — the register grew a `run_managed` runner (steps ≥2 managed squads head-to-head + drives towers) and two scenarios on the *managed* path (the ADR-0019 utility, vs the per-creep `run_engagement` most of the register uses): **EXP-POS-SELFPLAY-1** (two managed ranged squads close + fight cohesively) and **EXP-POS-KITE-1** (a managed ranged squad kites + kills a melee squad). **weight-sweep loop (`6f59b82`)** — a sweep over the kite preset's `w_future`/`w_prox` scored by the continuous net-HP exchange, run as a standing **regression guard** (trips if a scorer change leaves the default dominated). **Findings:** (a) on the melee beds (open / under-MOVE'd / healing-bruiser) the sweep response is **FLAT** — the outcome is invariant to these weights (the utility isn't brittle to weight choice in a melee standoff; positioning shifts tiles, not who-bleeds), so there's no tuning *gain* there — a weight-DISCRIMINATING bed (terrain-/tower-pressure, damage-taken continuous in position) is the Stage-4 follow-up where the sweep will actually tune; (b) EXP-POS-SELFPLAY is **low-casualty** — two ranged squads reposition rather than sustaining range-3 trade (an **engage-stickiness** tuning target). decision 59 / agent 35 / eval 15; clippy + check-wasm clean; no WFV bump. **Deferred (opportunistic follow-ups, NOT open stage scope):** the boosted-TOUGH net-damage conversion in `ThreatField` (needs a `boost` field plumbed through the body DTO → most-fragile member's damage-reduction applied at the safety term/veto; v1 is unboosted = safe-conservative, over-flees a boosted brick), a terrain/tower weight-discriminating sweep bed, and the engage-stickiness fix. None of these gate the ADR (it is Accepted/COMPLETE); pick them up only if live tuning demands.
...
> This document is the implementer's contract. It **supersedes** the proposed synthesized spec. **As of 2026-07-01 the contract is fully realized: all stages (0–4) plus §8 are on master and deployed; §7 (the "approve Stage 0+1" gate) and §5's per-stage approval requests are HISTORICAL — kept for design lineage, not open asks.**
...
## 9. Resume checkpoint — combat live soak (2026-06-23) [SUPERSEDED 2026-07-01]

**Current state (2026-07-01):** this checkpoint is historical. The "watch a winnable
clear" acceptance and the MMO-deploy gate below were carried forward and CLOSED by the
later combat program — ADR 0027 (objective-squad lifecycle), 0031/0031a/0031b
(capability-driven force composition, replacing the tower-count proxy of item 3), 0032
(EV-optimal squad assignment), 0034 (rally/travel convergence), 0035 (scout-before-commit
+ abandon-on-contact), 0036 (opportunistic structure targeting), and 0037 (tower-aware
neighbour defense) all landed and **deployed to MMO**. Squads reach, engage, breach, and
defend live. The per-item annotations below record how each open item resolved.
...
2. ~~**Watch a winnable clear end-to-end**~~ **DONE / SUPERSEDED 2026-07-01** — the
   travel → breach → dismantle (or correct Lanchester retreat) pipeline was hardened
   and proven by ADR 0034 (rally/travel convergence) + 0035 (scout-before-commit /
   abandon-on-contact) and is live on MMO; squads reach + engage end-to-end.
3. ~~**Winnability gate is a tower-count proxy**~~ **RESOLVED 2026-07-01** — the
   `MAX_SINGLE_SQUAD_STRONGHOLD_TOWERS` proxy was replaced by the principled
   force-vs-defense assessment: ADR 0031/0031a/0031b (capability-driven force
   composition) + ADR 0032 (EV-optimal squad assignment) drive target selection and
   sizing; ADR 0036 (opportunistic structure targeting) + 0037 (tower-aware neighbour
   defense) round out structure/tower handling.
4. ~~**MMO deploy** still gated per ADR 0020 §10~~ **DEPLOYED 2026-07-01** — the
   combat program (rally/engage/structure/tower-defense per ADRs 0027/0031/0032/0034/
   0035/0036/0037) is live on MMO. This ADR's position-utility ships as its substrate.
