...
- **Status:** Accepted; **Stages 0–3 LANDED 2026-06-19 (host-validated, NOT deployed)** — the unified position utility is live in `decide_squad_with_pathing` as the default (no kill-switch). Stage 4 (incremental, measure-first tuning via the EXP-* sim loop) is the only remaining stage and is opportunistic — there is no fixed scope to "approve," just ongoing weight-tuning + new EXP scenarios as combat work continues. The headline status line per-stage is the **(latest first)** log below.
