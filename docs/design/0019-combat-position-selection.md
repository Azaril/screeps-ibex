...
- **Stage 3a DONE (`28d6540`):** `PositionLayers` (owns the threat-reachability + centroid-distance floods) + `PositionLayers::build` (use-agnostic — centroid + threats + matrix) + `.fields()` borrowing view; `plan_kite_anchor` refactored to build it once (byte-identical — same floods). +1 reuse test (build once, reweight per use). decision 49. **Remaining Stage 3b (default-ON, gated):** the CPU compound-worst-case bench (build first), the integer `ThreatField` + `focus_damage` layers, the signed normalized `position_utility`, `plan_engage_anchor` (attack-positioning) reusing the same `PositionLayers`, the 6 must-fixes, and a full sim EXP-COHESION. **No kill-switch flag** added (operator: not deploying until combat is better → harness-validation suffices; a live flag can wrap it near deploy). gates: engine 40 / rover 31 / decision 47 / agent 35 / eval 13; clippy + bot wasm-check clean; no WFV bump.
