...
2. **Global layer duplicated per room.** `draw_global_layer` is drawn into the global target *and re-drawn into every room* (visualization.rs:1405) — ~200–350 redundant primitives/tick at 10 known rooms — the IBEX-008 duplication.
...
**Verdict ledger (2026-06-11 root-cause investigation — do not re-flag):** refuted as the corruption-mode cause: the 500 KB limit-throw (it is Mode A — real, loud, volume-dependent, never single-primitive), current-toolchain shape divergence (all 10 primitive paths byte-identical to the engine's JS producers; live Redis verified), the `to_value()` panic, packed-`Position` on the wire, tagged-enum→ES-Map→`"{}"` (unreachable via the current API, and `"{}"` is skipped not fatal — Mode C). Still open: which injector produced Field Report H (no payload was captured; predates-the-toolchain, PR #481 class, and the B2 dead-subscription masquerade are the candidates). The operator's live observation — corruption appearing to originate in the world/`RoomVisual` paths — points the same way as the Mode B analysis. Coverage gap on record: P1.C6 guards only the `RoomVisualizer` path — `draw_claim_map_visuals` (a world/map `RoomVisual` path) bypasses clamp, cap, and buffering, and is not exercised by the smoke pass; it is the strongest candidate for "one world primitive breaks all rendering", and the render-acceptance probe on the private server is how its payload gets captured. Movement visuals (per-room, clamped, literal styles) are not exposed to Mode B.
