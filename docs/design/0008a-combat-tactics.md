...
**Register status (2026-06-19):** the register is now a runnable, metric-producing suite in the **`screeps-combat-eval`** crate (P2.H5): `register()` runs each experiment through the H4 sim (`run_engagement`), extracts measured metrics, and gates them; `report()` is the tuning dashboard. **Run + passing:** EXP-FOUND-1 (kill inequality), EXP-KITE-1 (0 melee at MOVE parity), EXP-FOCUS-1 (3×ranged clear a turtle in 3t), EXP-TOWER-1 (edge drain bleeds the tower to 0), EXP-COMP-1 (DPS-vs-heal-wall comparison: quad 5t < duo 15t). Plus EXP-SQUAD-KITE-1 (the earlier G3-tail self-play validation, in `screeps-combat-agent`). **Still pending:** FOUND-2, KITE-2, FOCUS-2, TOWER-2, BREACH-1/2 (need sim ramparts+repair), the full COMP-1 uniform-brick-vs-2+2+TOUGH sweep, DEF-1/2, ENGAGE-1, NPC-1, CTRL-1 (need controllers), PARITY (server). Tracked as EXP-REGISTER in the [master plan doc](../plans/combat-overhaul-plan.md) §5.
