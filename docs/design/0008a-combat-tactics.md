...
- Default quad composition: ship the uniform RA+HEAL brick quad (16RA/14HEAL/6TOUGH per member, rotation-robust, degrades gracefully) or keep the bot's asymmetric 2-ranged + 2-healer split? Each uniform member is a weaker single-role unit. Resolve by EXP-COMP-1 — which kills a 25-HEAL boosted defender faster AND survives a 6-tower nest longer — but the operator should set the default the sim optimizes toward.
- Boost-commit policy: the boosted in-bunker / siege tactics (T-TOWER-3, T-COMP-1/5, T-NPC-7) presume the boostqueue can supply XLHO2/XGHO2/XZHO2/XKHO2 in quantity. When the lab economy is short, the NO-GO threshold collapses to the unboosted line (N≥2 in-bunker) and squad selection must downgrade. Operator decision: what stored-mineral / lab-throughput floor gates a boosted assault vs an unboosted one (or vs waiting)? ship the uniform RA+HEAL brick quad (16RA/14HEAL/6TOUGH per member, rotation-robust, degrades gracefully) or keep the bot's asymmetric 2-ranged + 2-healer split? Each uniform member is a weaker single-role unit. Resolve by EXP-COMP-1 — which kills a 25-HEAL boosted defender faster AND survives a 6-tower nest longer — but the operator should set the default the sim optimizes toward.
- Boost-commit policy: the boosted in-bunker / siege tactics (T-TOWER-3, T-COMP-1/5, T-NPC-7) presume the boostqueue can supply XLHO2/XGHO2/XZHO2/XKHO2 in quantity. When the lab economy is short, the NO-GO threshold collapses to the unboosted line (N≥2 in-bunker) and squad selection must downgrade. Operator decision: what stored-mineral / lab-throughput floor gates a boosted assault vs an unboosted one (or vs waiting)?
