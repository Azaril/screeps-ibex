...
GPL accrues and is never spent: a crate-wide `PowerCreep` search still finds **zero operator code** (0013 "G-2c: nothing" — re-verified 2026-07-10; the only matches are sim constants and a rover impl detail). The accrual plumbing is live — `PowerSpawnMission` (`missions/powerspawn.rs`, launched by `ColonyMission` at `missions/colony.rs:246-253`) hauls energy+power and calls `process_power()`; `TransferTarget::PowerSpawn` is frozen end-to-end; GPL level/progress are already in stats (`statssystem.rs:211,234-238,312`). Since 0013 was written, the economy grew a real currency (ADR 0040: integer **milli-e/t**, storage = par = 1000), a pure decision seam (`screeps-econ-decision`), a numeric-bid ticket lane (WFV 27, live on MMO), and a reduced-cost haul-admission model (0044). 0013's operator half (D3) predates all of that; this ADR is the market-era spec that makes it buildable — and adds the piece 0013 never designed: **surviving our own WFV resets**.
...
**D3.2 `power_process_bid` — the kernel that converts ADR 0043 B4.** Today `powerspawn.rs:87-147` registers its energy demand through the coarse `TransferDepositRequest::new_tier` band path, sanctioned by 0043 B4 *only because* "no EV kernel exists." Mint it:
...
| Defer all of this until 0013's bank-farming half is proven | One dependency chain | Processing + operators don't require bank kills (slow accrual + 0012 power purchases feed GPL); the combat stack is live-but-unproven (2026-07-09 review) and gating on it idles free GPL. The halves are independent by 0013's own design |
...
- Band/currency: `spawn_policy.rs:39-58` (`SPAWN_BID_*`, `BID_SCALE`); 0043 scale-migration correction (2026-07-08) — the mixed-scale trap.
- WFV mechanics: `screeps-ibex/src/game_loop.rs:746` (`WORLD_FORMAT_VERSION = 27`), `:753+` (`deserialize_world` loud reset), `segments.rs:62` (segments outside WFV).
