...
`optimize_composition(req, defense, enemy, target_value, member_energy, params) -> Option<SquadComposition>` runs ONE parameterized, bit-deterministic search over creep splits (n_fighters × n_healers, `1..=8`) × over-power factor; commit iff `max EV > commit_threshold`, else `None`. `CompositionParams` (NOT `Serialize`) carries the **four tournament-tunable knobs**: cost weights, safety margins (`hold_margin`/`over_power`/`dynamic_margin`), member-energy split, and commit threshold. `emit_requirement` (T1) survives unchanged as the optimizer's win-target; `win_probability` (`force_sizing.rs:366`) survives as its probability model. The currently-fixed seed constants `HOLD_MARGIN` (`:27`), `COORDINATED_DPS_MARGIN` (`:34`), and `IMPORTANCE_MAX_EXTRA` (`:386`) migrate into `CompositionParams` and become swept knobs. Small-many-vs-few-big and over-power level **emerge** from the tuned weights — ONE search now; codifying emergent strategies as explicit selectable strategies is a documented follow-up.
