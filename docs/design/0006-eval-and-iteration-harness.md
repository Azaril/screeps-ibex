...
- **Layout** — where should harness scripts + run artifacts live (`tools/eval/`, `runs/`)? **Resolved (amended by Phase 0):** in-repo crates (now `screeps-server-kit/` + `screeps-ibex-eval/` after the P0.A14 split), **workspace-EXCLUDED** (not a member — the workspace's wasm32 default target would break a tokio/bollard host crate, and a member would break workspace-wide `cargo check`; the crate carries its own `.cargo/config.toml` pinning the host triple, per the foreman-bench exclusion precedent). Artifacts under `runs/` keyed (scenario, git SHA) — pinned in component-test-plans §15/F14. See [`../execution/phase-0.md`](../execution/phase-0.md) P0.A1.
