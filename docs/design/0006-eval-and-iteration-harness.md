...
> **Forward note (2026-06-29) — ADR 0033 extracts + renames Part B's mechanism layer.** By P-MOVE+ (2026-06-24), Part B's `screeps-combat-engine` is the shared two-halves mover described below: rover *plans*, the engine *applies*. [ADR 0033](0033-rover-pathing-sim-and-benchmark.md) (Proposed) factors the engine's **combat-agnostic** movement mechanism (world / terrain / body / contention / fatigue / edge-exit / recording) into a new lower crate **`screeps-sim-core`**, with a full `Combat*`→`Sim*` rename (`CombatWorld`→`SimWorld`, `resolve_tick`→`resolve_movement_tick` + `resolve_combat_tick`, `CombatRecording`→`SimRecording`). After it, `screeps-combat-engine` = `screeps-sim-core` + the combat layer, and the `CombatWorld`/`resolve_tick`/`CombatRecording` names below read as their `Sim*`/split successors. The harness architecture this ADR decides is unchanged.
...
**Movement architecture — rover is the one intent system; the engine is the server (operator decision, 2026-06-17).** Movement is routed through `screeps-rover` everywhere — *rover drives intent* (path planning, caching, stuck/repath, the dormant `Follow`/pull, and the `MovementVisualizer` so the sim gets visualization too), and the **"server" provides correctness/validation**: live, the real Screeps server resolves the requested moves; in the sim, the `screeps-combat-engine` `resolve_moves` (a verbatim port of the server's `movement.js` `check`) is authoritative. So the **decision** (the tactical seam) AND the **per-tile search** are shared between live and sim; only the final conflict resolution differs in *mechanism* — and the sim's is the ground-truth engine port, which is *more* faithful than re-running rover's client-side coordination. This **revises** the earlier wording ("the sim reuses rover's `PathfindingProvider`"): concretely, rover is a workspace member with a `screeps`-gated JS adapter and a pure default core (the foreman pattern — `default = []` "compiles and runs on native targets … no JS"), but it ships *only* the JS `ScreepsPathfinder`; so the sim adds a **pure headless `LocalPathfinder`** (a weighted-grid A*/flee search over the caller-supplied `LocalCostMatrix`) **in rover** (not a one-off in the harness — the pathfinding system owns searches), plus a `CombatWorld`-backed `CostMatrixDataSource`. The footprint **"moving-maximum" W×H transform** (M1, generalizing the dead `apply_quad_cost_overlay`) lands in rover for the same reason. The host-only harness then depends on `screeps-rover` *without* the `screeps` feature (adding only the `screeps-common`/`screeps-cache` patches), never the whole bot.
