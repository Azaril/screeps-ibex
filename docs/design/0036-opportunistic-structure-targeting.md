...
and the focus/heal assignments — it does not read `decision.member_intents`, so the
...
1. **One target set, one currency.** Creeps and structures already share the kernel's
   ...
   honor the kernel's structure choice and approach.
...
3. **Functionally pure in the kernel.** The threat/value weighting and the
   ...
   the resulting focus.
4. **Opportunistic everywhere — no doctrine gate.** Phase B2 runs `compute_squad_orders`
   for *every* managed squad with the room's full hostile-structure list and a uniform
   `engage_objective = Destroy`, and `decide_squad → select_focus_target` picks a
   structure focus UNCONDITIONALLY. Any engaged squad — defense, SK, offense — therefore
   razes a hostile structure when it has no better creep target. Structure razing is a
   property of the target-valuation, not a privilege of one doctrine.
...
`structure_rank` (the fixed core>spawn>tower order) is deleted outright rather than
retuned — a second ordering is exactly how a live tower came to rank below a spawn.
`select_focus_target`'s structure fallback (lib.rs:291-296) and
`best_hostile_structure_within` (lib.rs:558-563) sort by `struct_target_value`
descending, so they pick the highest threat×value structure — towers-then-core in a
towered room, the core directly in a bare room. Creeps still come first via
`ev_target_order` (unchanged); the structure order is the tiebreak/sole-set when no
creeps are killable.

### D3 — LIVE: direct the structure focus (fixes G1 and G2)

`resolve_focus` keeps a `AttackTarget::Structure(pos)` as a focus of `(pos, None)`
instead of dropping it through `resolve_creep()`, so the job's `CombatView.focus` names
the kernel-chosen structure and `attack_with_orders` focus-fires it. The kernel's
value-sorted choice becomes the live behavior; the id-less focus is the signal that the
target is a structure (used again by D4).

*Alternative considered and rejected:* stamp each member's `decision.member_intents[i]`
onto its `TickOrders` and re-emit them through `translate_intents`
(squad_combat.rs:555-603, which already handles `Attack`/`RangedAttack` against
`id:None` structures by position — only `Dismantle` is a no-op there). That would close
G1 literally, but it re-plumbs the whole intent channel for a case the focus already
expresses; the focus form has a far smaller blast radius for the same live outcome.
...
anchorless path and each member moves to its kernel `member_goal`. The predicate is
`should_drop_anchor_for_structure_siege`, scoped to `focus.id.is_none()` — i.e. exactly
the id-less structure focus D3 produces — so creep formations are untouched. The
kernel's `dist_to_target` flood is already built from the structure focus
(lib.rs:2058-2067) and the approach term pulls members downhill to weapon range
(kernel.rs `best_tile`), so a squad parked at range >3 closes to range 3 and fires —
exactly the bare-spawn test (`a_melee_creep_breaches_the_focus_structure`,
kernel.rs:893) but with the live wiring honored.
...
Engine ground truth: `creep.dismantle()` is a hard no-op on `StructureInvaderCore`
(the structure has no `CONSTRUCTION_COST`), so the offense attack path is the *only*
path that can raze a core. The Dismantle objective carries the target structure's hits
so the oracle sizes ranged kill-parts against the real HP (ADR 0031 R-attack already
does this for cores). No composition change is needed for cores beyond confirming the
ranged ceiling sizes to the core HP; the bug was wiring/reach, not weapon selection.
(Dismantle-able rings keep WORK via `DismantleStructure`.)

The offense producer is what fields this: `war.rs:1195` pushes an `InvaderCore`
`AttackCandidate` for every scouted live core, and `war.rs:1386` routes it to
`KillImmuneStructure`/`Dismantle` (RANGED), with level-0 cores exempt from the
concurrency cap.

**Objective routing, not raze failure.** A no-creep, undeployed core produces no hostile
creep, so `hostile_warrants_defender = false` and no `Secure` objective is emitted for
that room; the core reaches a squad through the offense `Dismantle` arm (war.rs:1386).
A squad that never arrives at such a core is an objective-*fielding* question, not a
targeting one.

**Deliberately out of scope:** a far (>1 hop) / sourceless level-0 *reserver* core gets
no offense candidate at all (`invader_core_attack_score` → `None` when `!wanted_remote`).
That is the intended "let it self-collapse (~75k ticks)" behavior, not an omission — and
D6 does not strand it, because salvage re-admits the room once the core is gone.

### D6 — salvage-vs-offense routing

A derelict-marked invader-owned room must not be admitted by Salvage while a live core
remains: salvage spawns WORK labor and a Declaim objective and never targets the core.
The decision is to **exclude such rooms from Salvage eligibility until the core is
razed**, rather than have Salvage emit its own Dismantle objective mirroring war.rs's
producer — one producer for the core keeps the ranged razing force on the offense (War)
path and avoids two sources of truth for the same target.

Mechanism: `salvage_eligible(&SalvageEligibilityFacts)` short-circuits
`if has_live_invader_core { return false }` ahead of *both* eligibility paths, fed from
`room_data.get_structures().invader_cores()` — a `#[serde(skip)]` live-only field, so
presence in it means a live core, never a stale memory of one. A live-core room is
deferred to the offense Dismantle (which razes it via D1–D4); a core-free or razed room
still salvages. There is no lockout: the room re-qualifies as soon as the core is gone.
...
1. `struct_target_value` orders an energized tower ABOVE a spawn and below/above the core
   per the chosen weights; a drained tower drops to 0.
2. `select_focus_target(hostiles=[], structures=[core])` returns the core (the existing
   guard at lib.rs:2307); plus `structures=[tower, core, spawn]` → focus = highest
   threat×value (tower in range, else core).
...
Bot-crate tests (D3/D4 are live-wiring and are structurally unreachable from the
combat-eval, which does not depend on the bot crate and whose `ManagedSimSquad` is
anchorless — the sim-gap class):
5. `resolve_focus` keeps `(pos, None)` for a Structure target.
6. The `should_drop_anchor_for_structure_siege` predicate.
7. A real `apply_squad_decision` Engaged-arm case asserting `squad_path = None` and
   `attack_target = Structure(pos)`, with a creep-focus control.

Engine/harness coverage (combat-eval):
8. A scenario: our force-sized ranged quad vs a BARE level-0 core (no creeps). Assert the
   core's hits strictly DECREASE over N ticks and reach 0 — the acceptance criterion for
   D3/D4. Add a towered variant: tower hits → 0 first, then core.
9. Reuse the deterministic sim fence (per the memory sim-determinism note) so the raze
   test is bit-stable.
...
- **Salvage routing:** D6 keeps cores on the offense path; the declaim path neutralizes
  the controller only.
- **Serialized shape:** D1/D2 are pure-crate (no serialized shape). D3 uses an existing
  ephemeral `TickOrders` field, threaded through the same non-serialized order channel
  `member_goals` uses. D4 drops the anchor at RUNTIME (the same pattern as the
  DRAIN/rally/skirmish anchor-drop, squad_manager.rs:2526). D6 keys off a
  `#[serde(skip)]` field. The design therefore carries no serialized-shape change.

## Landed
- `64aee70` / `690fdb6` / `7bdf1fd` / `b63c7fa` D1–D5: threat×value structure valuation, focus order, live structure focus + siege anchor drop, raze mechanism (2026-06-30)
- `2cdf636` D6: salvage defers live-invader-core rooms to the offense raze path (2026-07-01)
