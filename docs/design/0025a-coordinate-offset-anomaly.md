# ADR 0025a — Terrain coordinate convention + the residual object anomaly

- **Status:** Decided

The screeps room-terrain string is decoded **column-major**; the "coordinate mismatch" that motivated
this ADR was a decode **transpose** (row-major vs column-major). A smaller **residual anomaly** remains
an open question (~15–20% of objects read as wall under the corrected decode, with no global transform
fixing them); it is mitigated by `snap_to_open`.

## 1. Resolution: the terrain string is COLUMN-MAJOR

The screeps `/api/game/room-terrain?encoded=1` string is decoded **column-major** (`index = x*50 + y`),
matching the `screeps-game-api` `LocalCostMatrix` `xy_to_linear_index` — **not** row-major (`y*50+x`) as
the rest-api comment and python-screeps docs claim. A row-major decode silently **transposes** every
imported room, and a `snap_to_open` step then compensates by moving the (now-misplaced) objects to
adjacent open tiles in the transposed frame.
...
- The screeps+ dump's terrain string for E11N1 is **byte-identical** to the official API, and the object
  ...
  bug" conclusion recorded in ADR 0025).
...
The convention therefore is: `decode_terrain`/`decode_fast`/`encode_terrain` use `x*50+y`. `decode_fast`
transposes into the `FastRoomTerrain` row-major buffer (`buffer[y*50+x] = string[x*50+y]`) so the foreman
planner and the sim agree. Foreman base captures must be taken on the corrected terrain.
...
that is what `decode_terrain` does. (This is also why a naive row-major decode put 100% of objects on
...
## 2. Residual anomaly (open design question)
...
convention/transform. Candidate explanations to chase:
...
**Mitigation:** `terrain_import.rs::snap_to_open` snaps each object to its nearest open tile at
...
screeps-combat-eval/resources/real-terrain.json
        │  terrain_import.rs
        ├─ decode_terrain / decode_fast  → x*50+y  [column-major]
        └─ fixtures(): snap_to_open(objects)  ← safety net for the §2 residual (no-op for most)
        │
        ├─► Stage 2 ImportedRoom: base anchored from the (correctly-oriented) terrain
