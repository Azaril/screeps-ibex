...
- The combat sim (ADR 0023) **runs rover but never measures it.** Since P-MOVE+ (task #30) the sim drives the *live* rover stack — `MovementSystem` + resolver + `LocalPathfinder` + `AnchorPath` — to produce each tick's move directions (`resolve_moves_via_system`, `pathing.rs:477`; the single-creep `resolve_move_direction`, `pathing.rs:198`), then hands them to the combat-engine's `resolve_tick` and scores the *fight* (DPS, healing, cohesion, survival). Rover is **exercised on every tick, measured on none** — no combat metric grades its path optimality, fatigue/time-moving, congestion, ops, or determinism.
...
#### §D1a — `screeps-sim-core`: the pure movement kernel

> Earlier drafts explored other shapes (`SimWorld` + a separate `CombatState`, a `resolve_combat_tick` rename, no `CombatWorld` type); the decisions below supersede them, and the [Alternatives](#alternatives-considered) records why.

`screeps-combat-engine` bundled two separable concerns: (1) **general Screeps movement mechanics** — terrain, body-sizing arithmetic, the same-tile contention resolver, fatigue, edge-exit, the per-tick movement world — and (2) **combat** — damage, towers, ramparts, controllers, safe-mode, body-combat powers. (1) belongs in `screeps-sim-core`; combat-engine keeps (2) and depends on the kernel. The kernel carries **no combat concept**.
...
**The combat layer (`screeps-combat-engine`)** — a thin layer on the kernel, **byte-identical** to the pre-extraction tick (the extraction's hard guard):
...
- **Combat's `Intents` keeps its `moves` / `pulls` maps** and `resolve_tick` builds a `MoveIntents` from them to call the mover. (Having `Intents` *embed* a `MoveIntents` is a small clean-up; the type-level guarantee "a movement sim can't emit a combat action" already holds via `MovementSim::Intents = MoveIntents`.)
...
| layering | `Simulation` trait + `MovementSim` | `CombatSim` impl (clean-up) |
...
**Boundary notes:**
- the offline **rover driver** (`resolve_moves_via_system` + `SimMoveRequest` / `SimMoveGoal` / `SimMoveCache`, ex-`screeps-combat-agent/src/pathing.rs:240-518`) lives in `screeps-sim-core` under an optional **`rover` feature**, so both the combat squads and the bench import one driver;
- the deterministic **`Rng`** (SplitMix64) is unconditional kernel surface, so both harnesses share one stream;
- combat-engine's `CombatRecording` is combat-coupled (records damage / deaths / tower intents) and therefore stays in the combat layer; the kernel carries the lightweight movement-trajectory recording;
- the **roads** + **loaded-CARRY** physics fixes are kernel-level (§D3).

**No debt:** git submodule + git-dep + root `[patch]` (no path deps); explicit composition (no `Deref`); direct cross-crate imports (no re-export modules / no constant bridge); no combat concept in the kernel.
...
screeps-sim-core   ── MovementState · SimTerrain · SimBody(movement) · contention resolver · resolve_movement · Simulation/MovementSim
   └─▶ screeps-rover            (feature "rover" — the offline driver module)
...
screeps-rover-eval   (host-only, NET-NEW measurement layer)
   ├─▶ screeps-sim-core[rover]   ── drive MovementSim over MovementState (+ the offline driver)
...
3. `sim_core::resolve_movement(&mut world, &MoveIntents::from(dirs))` **applies** them with engine fidelity (REUSED, **not re-ported**):
   ...
   - **Pull bypass** — a pulled creep moves even with nonzero fatigue / no MOVE. (The E-family pull-train scenarios require `pull` / `move_pulled_by` to be live in the shared driver, ex-`pathing.rs:337-342` — a SHARED capability that benefits combat too.)
...
> **The two shared physics fixes (both live in `screeps-sim-core`; both default-inert; both improve combat fidelity too).** The engine port omitted two fatigue terms that are harmless to combat but fatal to a hauler benchmark; they live in sim-core's `terrain.rs` / `body.rs` and benefit **both** sims.
>
> **SHARED-FIX-1 — Roads.** The pre-fix `fatigue_rate` (ex-`state.rs:22`) returned only swamp-or-plain, and `FATIGUE_RATE_ROAD` (`constants.rs:44`) was a **dead constant** (defined, never referenced). `SimTerrain` carries `roads: HashSet<(u8,u8)>` and `fatigue_rate` checks it **first**: road 1 → swamp 10 → plain 2 (engine `movement.js:204-213` order, road-wins-over-swamp). `resolve_movement` already routes fatigue through `fatigue_rate`, so the tick itself is unchanged; `ScenarioBuilder` + the cost sources place roads and wire rover's already-present-but-empty `roads` `LinearCostMatrix` so the pathfinder *prefers* them. Roads are the single biggest base-ops fatigue lever; without this every fatigue/time-moving metric over a built base is fiction. `is_wall` documents the one deliberate divergence (engine `checkObstacleAtXY` treats `wall && !road` as passable, but a road can't be built on a natural wall, so it's unreachable — walls stay unconditionally blocking). rover-eval's `cost.rs::fatigue_rate` **delegates to the kernel `SimTerrain::fatigue_rate`** rather than duplicating the swamp/plain branch, and its `TerrainCostSource` populates rover's `roads` sub-matrix (priced at `CostMatrixOptions::road_cost` = 1) *and excludes road tiles from the swamp overlay* so the apply-order (roads then `other.apply_to`) leaves road-on-swamp at 1. Pinned by `terrain::road_overrides_swamp_and_plain`, `tick::road_step_accrues_less_fatigue_than_plain`, rover-eval `metrics::road_corridor_across_a_swamp_is_taken`.
>
> **SHARED-FIX-2 — Loaded CARRY.** The pre-fix `fatigue_weight` (ex-`body.rs:155`) counted only non-MOVE/non-CARRY alive parts — CARRY was **always free**, with no notion of carried resource. Engine truth: an *empty* CARRY is free, a *loaded* one counts. `SimCreep` carries **`carry_used: u32`** (total resources aboard = engine `_.sum(creep.store)`) rather than a pre-counted `loaded_carry_parts`: modelling the store is strictly more faithful, because the boost-aware part accounting is then *derived* rather than assumed. `SimBody::carry_weight(carry_used)` ports the engine's `calcResourcesWeight` (`movement.js:41`): walk the body back-to-front, each ALIVE CARRY part absorbs `CARRY_CAPACITY × BoostTier::carry_capacity_mult` (×1/2/3/4) of the load and adds 1 weight until the load is exhausted (naturally capped at the alive-CARRY count). The full weight is **`SimCreep::fatigue_weight()` = `body.fatigue_weight()` (structural) + `body.carry_weight(carry_used)`**, called by both the mover's `rate4` contention denominator (`movement.rs:156/174`) and the tick's accrual (`tick.rs`) — so loaded-hauler *contention* is corrected in the same stroke. Loaded CARRY is the *dominant* fatigue term for haulers, the primary subject of this benchmark. Pinned by `body::carry_weight_counts_loaded_parts_only` / `carry_weight_is_boost_aware`, `tick::loaded_carry_accrues_fatigue_an_empty_hauler_does_not`.
>
> **Default-inert ⇒ combat byte-identical.** Empty `roads` + `carry_used = 0` make both fixes no-ops for every existing combat scenario (combat creeps carry nothing; combat rooms have no `roads` set), so the combat crates take only a mechanical `carry_used: 0` on their `SimCreep` literals with unchanged behavior — the combat suites re-passing unchanged is the inertness proof.
...
**Oracle ↔ executor cross-validation (guards a shared off-by-one):** the closed-form and time-expanded oracles can agree with each other while *both* embed a wrong sub-tick convention. So the oracle's per-tile stall is additionally asserted equal to the **server half's** observed stall on a single straight corridor — validating the oracle against the *engine ordering* (via the executor), not merely against the other oracle.
...
#### §D5.4 The objective function: `w(creep)` — the per-movement-intent value weight
...
**The eleven calibration points, each decided with its rationale recorded in `value.rs`'s `## §D5.4 decisions` block (each is one constant or match-arm, so a re-decision is a one-line diff):** (1) squad bid semantics — the **binding member bids the full `R_O`** for contention triage, per-member cost-shares are the sample weights; both integrate to `R_O` exactly once; (2) slack floor = the **amortized upkeep** `body_cost/1500/r`, not a literal 0; (3) the policy constants `ε_intel`/`v_sink`/`t_ramp`/`S_ref` ship as declared defaults with a sensitivity sweep behind them (rank-affecting points: `t_ramp` 10/40, `s_ref` 200, `v_sink` 0.5; `ε_intel` is inert — floored by upkeep); (4) ttl plumbing is **scenario-level** (zero kernel change) rather than a `ticks_to_live` field on `SimCreep`; (5) the economic-facts bridge is **scenario-supplied pre-priced facts** (the ObjectiveIntel pattern) rather than extracting `room_net_roi`/`claim_value` into a pure crate — realized by `annotate.rs`, which builds `GoalAnnotation`/`SquadRef` from the REAL `force_sizing`/`objective_value` kernels and proves the V_O-integrates-once property; (6) non-energy cargo uses **pinned constants** (seed-stable) rather than live FairValue; (7) renewal ttl semantics; (8) escape-bid magnitude = `max`, with a wounded-creep discount; (9) runtime adoption — quantized `w` **replaces** rover's High/Normal/Low resolver priority, so the benchmark's kernel is *the* triage rule the bot ships (gated on the combat-corpus adjudication, §D12); (10) multi-room `T*` approximation — cross-room assignments score against a **solo-`T*` baseline** (`t_star_rtt_solo`: the same assignment run uncontended through the same driver+config, so η is pure contention loss), the single-room oracle staying the stricter baseline where it applies; (11) H reporting granularity = per-family primary + pooled secondary.

**Live alignment of the gates.** `CLAIM_ARRIVAL_MARGIN` is **50** (`utility.rs:66`) so the claim reach gate matches the live claimer's own margin, and the survival veto is a **strict `>`** (`kite.rs:713`) so an exactly-lethal incoming still counts as survivable. TOUGH and CLAIM parts carry no capability class of their own, so their cost-shares **redistribute to the carrying member** (body-based) — that is what makes a squad's sample weights integrate to exactly `V_O`.
...
### §D9 The resolver occupancy model — "a creep that cannot move is still an occupant"

The benchmark's failed-move sentinel exists because an *issued* move the engine rejects is CPU spent for nothing ("intent spent, no action taken"). Reconciling every issued `Direction` against the tick's `MovementReport.moved` classifies each rejection: **`failed_fatigued`** (issued for a fatigued creep — avoidable), **`failed_wall`** (a pathing defect), **`failed_coordination`** (lost an own-fleet tile contest or a chain block — the *unanticipated coordinated movement* class; every sim creep is rover-controlled, so there is no NPC excuse), and **`failed_into_parked`** (walked into a creep that left the request set on arrival). `failed_move_rate = failed/issued` is the offline analogue of the live **G-13 wasted-move canary**. **Standing gate: `failed_coordination == 0` everywhere** — any unexplained rejection fails the build — with `failed_into_parked` pinned per scenario.

Holding that gate forces one structural invariant on rover: **a requested creep that cannot execute a move this tick must still be modelled as a stationary occupant**, not dropped from the resolver's occupancy map. Dropping it makes the tile invisible to the grant / avoidance / shove maps, the resolver plans other creeps straight through it, and the engine rejects every such intent. The same hole has four instances — (a) **fatigued or spawning** requested creeps (a fatigued creep occupies its tile through the whole movement phase, engine `canMove`; loaded all-swamp is 5 ticks/step, so most ticks are fatigued-in-place and the hole is loud), (b) creeps whose desired next step is **cross-room**, (c) creeps whose path search **errored** (`PathNotFound`), and (d) **arrived** requests (static miners, satisfied Flee). The invariant: `stationary_occupant` — non-displaceable (`allow_shove: false`, no desired tile), issued **no** intent, always visible. *Consent governs displacement, never visibility.* Synthesized shoveable-idle entries are fatigue-guarded for the same reason (a fatigued idle cannot evacuate, so consenting to a shove would waste the shover's intent and its own).

**Border ground truth (why (b) is a rover defect, not an audit artifact).** The engine's edge relocation is **unconditional** (`creeps/tick.js`): any non-NPC creep standing on an exit tile at end of tick crosses to the mirror tile, moved or not; and an off-edge move intent is **dropped at registration** (`move.js:32`). A contention-blocked crosser therefore bounces back onto the seam each tick, its next path step is the adjacent room's mirror tile, and issuing that as an outward `Direction` is a guaranteed-rejected intent every bounce — live too (the `aaac0f7` "move-to-room border thrash" family, at the rover layer). **The cross is the relocation, never a move.**

**Two claim invariants keep own creeps from double-booking a tile:**
- The grant path treats a resolved `final_pos` as **firmly claimed**, exactly as the avoidance paths always did. Swaps resolve first and leave the resolver's intent/occupancy maps, so without this a swap-ENTERED tile reads as free and a second own creep is issued into it — the engine admits only the swap member (mutual swap `rate1=100`) and wastes the other intent.
- `try_shove`'s frame-entry `firmly_occupied` snapshot goes stale across chain recursion, so a deep chain member could land on a tile the active chain had already promised to its caller. The **`freed_tiles` chain stack** forbids any member landing on a tile the active chain is vacating, so the chain picks a different landing and **succeeds** — fewer failed shoves, not merely detected ones. Post-recursion re-checks at all three assignment points stay as defence in depth.

### §D10 Stuck detection, repath cadence, and the ops pool

- **An absent budget means UNLIMITED, not exhausted.** `MovementSystem`'s CPU/repath budget checks read `is_some_and(exhausted)`, consistent with `is_over_tick_limit`. The opposite reading (`is_none_or`) silently disables **all** stuck escalation and expiry repathing for any headless driver that never sets a budget: a blocked creep re-issues its rejected move forever — linear intent burn, the live "lifecycle hang / CPU death-spiral" signature. Offline the drivers state unlimited budgets explicitly (there is no CPU meter offline; work stays bounded by the deterministic ops budget).
- **Repathing is an episode, not a level.** A naive "every immobile creep re-searches every tick" saturates the 20k/tick ops pool. The cadence is every-tick **inside** the designed escalation window (`ticks_immobile ≤ report_failure`) — fast recovery there is load-bearing (full-range exponential damping regresses the combat drain-soak bed to a wiped roster) — and **exponentially spaced per episode past tier 4**, with `repath_count` repurposed as the episode exponent (`StuckState::should_stuck_repath_with` + `ticks_since_repath`). Window and base come from the request's own `StuckThresholds`, so the cadence is per-request by construction.
- **First paths get a reserved slice of the ops pool** (`NEEDS_PATH_OPS_RESERVE_DIV`): repaths are *optional* work and may not consume the last fraction of the pool. Otherwise mid-order creeps run first-path searches on dreg allowances and stay budget-incomplete forever — and a pathless creep has no `path_data`, hence no stuck bookkeeping, hence no ops escalation and no tier 4: an identical doomed search every tick, forever.
- **`StuckThresholds` are per-request** (`Option` on `MovementRequest` / `SimMoveRequest`; `None` = the system default), which is what lets haul jobs run a slower, more patient ladder while military keeps the fast one, without either domain configuring the other.
- **Cadence and repricing are separate knobs.** `StuckThresholds::stuck_repath` is the stuck-repath *cadence* (and the storm damper's spacing base); `avoid_friendly_creeps` is *repricing only* (tier 1). They were historically one value (both 2) and could not be tuned apart; the split is byte-identical at the defaults and is pinned by `stuck_repath_cadence_is_decoupled_from_friendly_avoid_repricing`.
- **Escalation-tier shoving** is wired behind `set_shove_escalation`, unifying the resolver's `STUCK_SHOVE_THRESHOLD` into one tier-3 knob rather than two independent notions of "now try shoving".

### §D11 Registration, holding, and the numeric priority lane

**Registration is domain-dependent.** An unrequested hauler is parked junk that should be shoved aside; an unrequested COMBAT creep is usually *holding a position*. A single "register idle creeps" default that ignores this makes squadmates shove holders out of formation (a wiped-roster drain-soak outcome and >10% period-2 oscillation on the combat beds). The resolution is **holding-as-a-request**, not a per-domain flag: every living request-less squad member requests `move_to(own tile, range 0)` at `Immovable` — zero pathfinding (the arrival check short-circuits), enum-checked undisplaceable, never swaps — while civilians register as shoveable `Low` idles (`set_idle_creep_positions`). One registration default then serves both domains.

Two emergent hazards fall out of registration, each with its decided answer:
- **Proactive avoidance destroys the stuck signal.** A denied mover that sidesteps every tick never accrues `ticks_immobile`, so escalation never fires — a zero-failed-intent *dance* livelock that starves sealed corridors. Answer: **denial-as-stuck** (`ResolvedCreep::denied_by_idle` → `StuckState` records IMMOBILE even on an avoidance sidestep), so the dance feeds the ladder. Plus a **sealed-corridor fallback**: an idle-denial with no avoidance available degrades to the optimistic push, the engine-faithful last resort. Pricing held tiles impassable was tried and **rejected** — it over-corrects into `PathNotFound` stranding.
- **Bare holds turn a slam-stall into a period-2 dance around held tiles.** Answer: a **3-tick streak-gated doomed-goal damper** — a persistent range-0 goal on a squadmate's held tile converts to stand-and-fight, while transient pack-flow is untouched.

**Kite held-tile exclusion** (in `kernel.rs`, where `plan_squad_ev`/`best_tile` own member goals — not `kite.rs`, whose per-member layout the ADR-0025 clean-up deleted): exclude held tiles at candidate enumeration, gated to **proven** holders (an occupant already committed to `goal == its own pos`, i.e. exactly what becomes an `Immovable` hold downstream). Committed *movers* leave their tiles claimable so rotations stay intact, and *undecided* occupants do not block — blocking them measurably churns dense formations, so it is rejected.

**The numeric priority lane.** `priority_value: Option<i64>` with enum anchors Immovable / Low / Normal / High = −1M / 0 / 1M / 2M, so a quantized `w` in milli-e/t slots *between* tiers rather than replacing them wholesale. Military movement bids strictly inside `(Normal, High)` — above every civilian bid, below escorts' `High`: the squad's `R_O = p_win(live caps) · value_e / est_ticks` is computed once per squad per tick and stamped on exactly ONE member, the **binding laggard** (max range-to-focus among out-of-position members, lowest id breaking ties), which is what makes the healer a quad waits on outbid a fat hauler by construction. Civilian legs bid `quantize(energy / (2 × chebyshev))` — haul and harvest legs, upgrade/build pickup legs, link-mine deposit legs — with scout room-travel at the intel floor, claim/reserve travel at the `V/S_REF` floor, and build/upgrade work-travel at `WORK × 5/1` e/t.

**The combat friendly cost layer.** `CombatWorldCostSource::get_creep_costs` stamps same-owner tiles `u8::MAX` (self included — `screeps_impl.rs` verbatim), **escalation-only**, first paths keeping `friendly_creeps: false`. Engaged/anchored in-room members and formation movers carry an unreachable tier-1/1b (`u16::MAX`) on the default cadence: inside a tight formation the unstick lane is hold / shove / swap / denial-as-stuck plus a next-tick goal re-decide, **never** a detour around the cluster. Out-of-room travellers keep the default ladder and get the working friendly-avoid. The single-room `CombatCostSource` (kite scorer / anchor advance) stays friendly-transparent by design.

**G-13 alignment.** The live `move_failures` counter measures self-reported give-ups, which is not the wasted-move rate. The true rate is the same **issued-vs-moved reconciliation** the sentinel uses, taken at the `CreepHandle`-wrapper seam (an intent rover actually spent; position unchanged ∧ fatigue == 0 next tick; fatigued and dead excluded) and surfaced as seg-57 `pathing.wasted_moves` *next to* the retained give-up level so offline and live can be compared directly.

### §D12 Tuned defaults and the evidence behind them

- **`DEFAULT_REUSE_PATH_LENGTH = 20`** (rover, live). Path *commitment* defeats the repath-flap livelock that starved rooms at the stock reuse of 5 (completion 0.844 → 1.000, wasted intents 832 → 28, pooled H 0.680 → 0.769 on first measurement). Fewer expiry repaths also means less live CPU, and it is provably inert for changed-destination movement — `dest_matches` discards the path immediately, so kiting is untouched. Reuse 40 ties byte-exactly: commitment past one round trip buys nothing.
- **`StuckThresholds::default()` for combat, `ladder(8)` per-request on haul.** A 60-seed self-play adjudication (open + pinch beds, all-integer outcomes) found `ladder(8)` combat-**outcome**-neutral — no decisive fight flips, ticks-to-decision identical — but movement-quality-**negative** in congestion (pinch damage-traded −12%, later first blood) with no combat upside, while on the haul corpus it is worth a real H gain. Hence split defaults carried on the per-request lane (§D10) rather than one global ladder. The `report_failure` 12 → 48 worry dissolves on inspection: rover's tier 4 is contract-only in-process, and the live job layer has its own `STUCK_REPORT_THRESHOLD = 10` conversion.
- **`w`-as-resolver-priority passes its combat gate.** The same 60-seed adjudication, numeric value bids vs enum tiers: outcomes identical in aggregate (13v13 decided, 32v32 side-0, zero directional bias — flips only at undecided-stalemate margins), first blood *earlier*, damage **+5.4%** (pinch family **+8.0%** — the opposite sign of `ladder(8)`'s congestion cost). Re-run with REAL kernel-derived annotations it holds outcome-identical seed-for-seed against the fixture arm, which shows the effect is band-shape-driven rather than magnitude-driven. This is the gate live military-`w` adoption cites.
- **Idle registration ON as the single kernel default**, once holding-as-a-request exists (§D11) — one behavior across domains instead of a per-domain opt-out.
- **combat-eval `weight_sweep` GROSS guard = 1200** (re-pinned from 600). Holding-as-a-request (§D11) changed the *chase dynamics* the kite beds measure: chasers now sidestep-flank a holding packmate instead of closing, so the default kite weights sit on the **contact-lost branch of a 2-outcome bifurcation**, and the sweep's gross margin steps to the larger of the two outcomes. The guard is a robustness envelope around the branch the defaults actually land on, so pinning it at 1200 keeps the sweep an honest regression fence rather than a standing false alarm about a bifurcation. It is deliberately *not* a substitute for re-fitting the weights — robust kite-weight tuning belongs to the multi-bed tournament (the `param_sweep`/`tuning.rs` idiom over the kite weights, gated on the drain-soak + oscillation + Lanchester beds), not to this single-bed guard. **That tournament RAN 2026-08-23 (WS-4 `s4_weights_retune`, chokepoint basket, maximin-over-regimes): the shipped kite weights are confirmed — flat/insensitive surface, no adoption.**
...
- **The `sim-core` extraction is real, one-time engineering** — extracting the movement phases of `resolve_tick` into `sim_core::resolve_movement`, recomposing `CombatWorld` around an embedded `MovementState`, and the `Combat*`→`Sim*` rename of the kernel types touch `combat-engine` / `agent` / `eval`. The hard guard: the recomposed combat tick must be **byte-identical** to today's — the existing combat determinism fence (spread 0) + the FP/FN calibration are the gate (§M0). *After* it, the server half is **reused, not re-ported**, so there is no second physics half to drift from the engine.
...
Named, independently-testable slices, each with a gate (the 0023 S1–S5 / 0028 K0–K4 / 0006 Inc A–E idiom). **Bot serialized-shape / state-drop: None at any step** — host + sim code only, no `WORLD_FORMAT_VERSION` bump, no reset (`0015:204`). The one *internal* breaking change is `combat-engine`'s API in M0 (the world recomposition + the `Combat*`→`Sim*` rename of the movement kernel); all consumers are in-workspace and updated in lockstep.

- **M0 — Extract the `screeps-sim-core` kernel; make combat a thin layer.** Move `terrain` / `body`(movement methods) / `world`(`MovementState`) / `movement`(contention resolver) / the movement tick (`resolve_movement`) / `intents`(`MoveIntents`) / movement `constants` into `screeps-sim-core`, and add the `Simulation` trait + `MovementSim`. `combat-engine` becomes a thin layer: `CombatWorld { movement: MovementState, … }` (**composition, no `Deref`**), `resolve_tick` calls `sim_core::resolve_movement`, body-combat moves to the **`SimBodyCombat` extension trait**, combat constants stay in combat-engine, and **no re-export shims are kept**. New submodule + git-dep + root `[patch]`. **Gate:** every combat test preserved and the recomposed combat tick **byte-identical** to the pre-extraction tick; wasm-compat; clippy clean. As-built detail in §D1a.
- **M1 — Move the offline rover driver down (sim-core `rover` feature).** `resolve_moves_via_system` + `SimMoveRequest`/`SimMoveGoal`/`SimMoveCache` + `SimCreepHandle` + `SimMovementExternal` + `DEFAULT_SHOVE_DEPTH` live in `screeps-sim-core::rover_driver` (feature `rover`). **Design refinement over the §D2 sketch:** the combat cost source is **not** moved or renamed into the kernel. The kernel driver is instead **generic over the routing policy** — `resolve_moves_via_system<S: CostMatrixDataSource>(movement: &MovementState, requests, cache, cost_source: S)` — and `SimMovementExternal`/`SimCreepHandle` read **only `MovementState`**, so the kernel has **zero** combat coupling. `CombatWorldCostSource` (towers / ADR-0024 threat / structures) **stays in `combat-agent`** as its pricing policy, injected through a thin same-named `combat-agent::pathing::resolve_moves_via_system` wrapper; `from_intent` (which needs `CombatIntent`) becomes the free fn `move_request_from_intent`. The SplitMix64 `Rng` is promoted to the kernel (`sim-core::rng::Rng`, unconditional) and combat-eval de-duplicates onto it, so both harnesses share one deterministic stream. **Gate:** combat squad + mover integration behavior unchanged; full-workspace wasm build green (the rover driver is wasm-compatible).
- **M2 — Stand up `screeps-rover-eval` reusing everything (Tier A + oracle + A-family).** New host-only member; depends on `sim-core[rover]` + `combat-eval` (harness) + `rover`. First scenario: build a world via `ScenarioBuilder`, drive `resolve_moves_via_system`, apply via `resolve_movement`, capture via `record_tick`, render via `visualize`; reuse `terrain_import` for one real room; build `oracle.rs` (`gridsearch`-backed) + the A-family. **Proves the reuse wiring end-to-end before changing any physics. Gate:** a creep walks start→goal + replay renders; `R_fatigue ≤ 1+ε` / `R_len==1` (uniform) on A1–A6; A7 fails correctly; `det_repeat`/`det_reorder` spread==0 on the A corpus.
- **M3 — Land the two SHARED-FIXES in `sim-core` (roads + loaded-CARRY).** As specified in §D3, plus the Tier-A travel-time metrics the physics unblocks: `R_ticks` / `movement_eff` / `fatigue_util` over a kernel-driven single-creep `traverse.rs` route-walker that counts fatigue stalls tick by tick, and a **scenario harness** (`scenario.rs`) — a `Scenario` (terrain + body + route + metric gates), a curated `catalog()` (open / wall / swamp / road, under-MOVE, loaded, swamp-trap), a seeded procedural `generate()` over `sim-core::rng`, and a `validate()` that runs the oracle + metrics and checks the gates. `fatigue_util` is `1 − idle_free/ticks`, so it is `==1` for a lone creep by construction (the "rover blameless" invariant) and only becomes a live signal under Tier-B contention, where `idle_free` is traffic idle. **Gate (inertness):** the combat suites re-pass unchanged (the combat `SimCreep` literals take only a mechanical `carry_used: 0`); the B-family accrual deltas match `loadedCarry × w(t)`; wasm-compat re-confirmed.
- **M4 — Tier B contention + metrics + the determinism fence (D/E + the rover lens).** `crowd.rs`: `run_crowd` drives N creeps through the **real `MovementSystem` resolver** each tick (the M1 kernel driver + a `TerrainCostSource`, applied via `resolve_movement`), classifying every travelling creep-tick as move / fatigue-idle / **traffic-idle**; `congestion` = traffic-idle ÷ active creep-ticks (0 in free flow), plus a deadlock detector. Note on reading it: the resolver **shoves blocked creeps sideways** rather than idling them, so `congestion ≈ 0` even at a one-tile pinch — serialisation shows up as arrival-tick spread, not idle, and the metric is a regression sentinel rather than a congestion measure. The **failed-move sentinel** and the occupancy invariants it forces are §D9; the stuck/repath and ops-pool rules are §D10. `stats.rs` supplies the distributional substrate (weighted mean + seeded-bootstrap 95% CI + p5–p95 percentiles over `(value, weight)` samples, byte-reproducible via `sim-core::rng`). The rest of the Tier-B surface: `oscillation_rate`/`cohesion` lifted from combat-eval, a livelock (move-without-progress) detector, the D- and E-families (E needs pull live in the shared driver, §D3), and §D7 promoted to a corpus-wide gate (rover aggregate spread==0, `det_hash_seed`, cost-matrix bytes hashed). **Gate:** `deadlock==0`, `livelock==0`, `train_follow_success≥0.95`, the `idle_blocked` partition correct, `failed_coordination==0`, and the full fence spread==0 on every scenario.
- **M5 — the parameter TOURNAMENT, the objective function, and the tuned defaults.** With `H` + CI defined (§D5.4), rover's tunables become searchable: `MoverConfig` (shove depth / reuse length / ops budget / friendly distance / **`StuckThresholds`, injectable via `set_stuck_thresholds`**) threads `sim-core::resolve_moves_via_system_with` → the fleet driver → `tuning.rs`. The module generalizes **combat-eval's `param_sweep.rs` idiom** (pure scorer → gates-first deterministic ranked key → env-driven staged sweep as an `#[ignore]` test → determinism pin → upfront budget estimate) rather than `tournament.rs`'s self-play: configs are non-adversarial and are scored against a fixed corpus, not head-to-head. Stages run by leverage (coordinate descent): escalation → commitment/budget → resolver. The corpus is REAL as well as synthetic — a `capture_layout` bin plans foreman bases over combat-eval's 13 real `mmo:shard3` fixtures into a committed `captured-layouts.json` (all structure kinds, so roads feed the M3 physics), with `base_scenario` + `energy_traffic_fleet` (per-source container→storage hauler routes) and two multi-room border routes on top, for **20 scenarios**. Combat configs are adjudicated separately by self-play (`combat-eval/harness/mover_adjudication.rs`, 60 seeded matchups, all-integer outcomes), which is what keeps a haul-corpus win from silently costing combat quality. The tuned outcomes and their evidence are §D12; the CPU half is the `bench.rs`-shape op-counted + `Instant`-timed gates with scaling curves (via the `pub` cost source + `tick_stats()`), the `bin/rover_bench.rs` corpus CLI (fast/full, env-configurable, per-family + audit report), and the C/F/G/H families over 0023's edge-exit fixtures. **Gate:** both tournaments re-run, per-family ratchet floors held, scaling-curve slopes within baseline, fence spread-0, full wasm, offline ↔ canary definitions aligned.
...
- **rover** (the live mover): `DEFAULT_REUSE_PATH_LENGTH = 20`; `StuckThresholds::default()` (the `ladder(8)` variant is a haul-lane, per-request choice); budgets `None` = unlimited; the numeric priority lane, per-request thresholds, escalation-tier shoving (opt-in), idle registration + denial-as-stuck + shoveable idles + the sealed-corridor fallback.
- **sim-core `MoverConfig::default()`** (the offline / economy domain): reuse 20, registration ON.
- **combat-agent** (the combat domain): the same kernel default, made safe by holding-as-a-request (§D11) rather than by opting out of registration.
- **rover-eval `FleetOpts::default()`** (the benchmark harness): per-request `ladder(8)` on haul requests + value-priority triage ON.

**Regression-tracked baselines (seed 1).** Full corpus under the harness defaults: **pooled H = 0.9625, CI95 [0.9516, 0.9725], p05 0.759, completion 1.000, failed intents 0 (all classes), deadlocks 0**; per-family border 0.9636 / real 0.9654 / synthetic 0.9273; 13-room base-traffic H = 0.9654 CI [0.9535, 0.9761]. Scaling curves: lanes **N^0.75**, pinch **N^1.244** over N ∈ 1..64 (gate ≤ 1.5 / backstop ≤ 3.0), route-length flat at per-room charge granularity. The tuning path each default earned is the design's own evidence trail: **0.680 → 0.884 (reuse 20) → 0.920 (parked-creep coordination) → 0.946 (per-request ladder) → 0.963 (value triage)**, with completion 0.844 → 1.000 and wasted intents 832 → 0. The combat side is held byte-fenced throughout (spread 0), with the adjudication arms tick-identical and flips only at stalemate margins.

**Sentinel classes the corpus keeps watch on** (each with a named mechanism and a bounded gate rather than a suppressed metric): border exit-tile `failed_wall` on contended cross-room routes, roadless all-swamp contention `failed_coordination`, and dense-crowd pinch breakdown at high N — the CPU-death-spiral class the bench exists to catch. Their root causes and fixes are §D9/§D10; the checked-in gate is `dense_pinch_crowd_holds_all_gates` (N=48, all classes == 0), and the ratchet floors (`KNOWN_STARVED` empty — a room regressing into starvation fails) are what make a recurrence a build failure rather than a drift.
