...
- **P1 (refill exact inputs) — SHIPPED 2026-07-08.** Two parts, both landed:
  ...
  - **Live exact inputs — no-lag ordering solution.** `SpawnRequest.priority` already IS the body ROI in the shared `BID_SCALE` currency (M5b), so `top_blocked_roi` = the head-of-line queued bid — no `w` reconstruction. Threading it to the flush-time refill generator (which can only read `room_data`, never the `SpawnQueue` resource) is purely a **system-ordering** problem, not an inherent one-tick lag: the canonical generator flush is `build_econ_snapshot` at the top of `RunJobSystem`, which runs *after* `RunMissionSystem` + `SquadManagerSystem` have registered every spawn. A dedicated **`SpawnRefillPricingSystem`** (a shared `Read<SpawnQueue>`) sits between the last spawn producer and `RunJobSystem` and calls `SpawnQueue::publish_refill_pricing()`, writing each room's `RefillPricingContext { next_body_cost_e = cheapest queued body, top_blocked_roi_milli = head-of-line bid }` from the now-complete queue into the shared cell the refill generator captured at registration. The cell is the required bridge (the generator can't read resources) and mirrors the `structure_data` Rc precedent; refill is *not* separable into a demand-registering system because `execute_demands` prices it as one target-type inside the room's full lazy haul-demand scan. Same tick, fully fresh. No WFV (queue + cells are transient). The `.min(cap)` and the `REFILL_NEXT_BODY_COST_FALLBACK_E=300` stopgap are deleted.
