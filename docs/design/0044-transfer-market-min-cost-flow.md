...
- **P1 (refill exact inputs):** thread `top_blocked_roi` + exact cheapest-queued-body `next_body_cost` from the spawn queue into the live refill registration; drop the `.min()` atomically; cap → degenerate guard. **Likely a WFV bump** (DTO threading of the blocked-request info + any in-flight netting) — batch with the ADR 0043 A1/A8 all-sinks activation before one deploy reset.
  - **P1 CORRECTION (found reading the kernel, 2026-07-08):** the naive framing above is *wrong* — threading the exact `next_body_cost` does NOT make the floor sub-cap, so the `.min()` cannot simply be dropped. `refill_bid` uses the derived floor `instant_spawnability_premium = 1000 + 1000·lane_deficit/next_body` as the **lower** clamp bound (`top_blocked.clamp(floor, cap)`). On a deep lane the deficit ≫ any single body (RCL8 empty ≈ 12000 vs a ~300 cheapest body), so the floor explodes to ~40×par regardless of how exactly `next_body_cost` is measured — the external `.min(cap)` is what actually bounds it. The real fix is to **bound the premium's deficit input to the imminent-spawn portion**: `instant_spawnability_premium(min(lane_deficit, next_body_cost), next_body_cost)` ∈ `[par, 2·par]` (filling the *next* body is worth up to 2×par; energy beyond that is buffer, priced by `buffer_deposit_bid`, not the instant premium). With the floor bounded to ≤ 2·par, `top_blocked_roi` (the ROI of the body the fill unblocks, itself ≤ cap after the 6-1 body-cost clamp) becomes the true driver and the `.min()` demotes cleanly to the degenerate guard. This is a kernel-semantics change (deficit-capping) that MUST be sim-validated in P2 (arm A2) against the current capped-rate A1 — it is not a mechanical threading.
