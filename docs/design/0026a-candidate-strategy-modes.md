...
| `ranged_duel_kite` | **~~REJECTED~~ → ADOPTED as the `open_combat` profile (WS-4 R19 re-tune, 2026-08-23)** | June verdict (−329 on the ranged mirror vs `open_combat`, rank 17/61) was measured on the pre-Wave-A/B kernel over open-terrain beds. The R19 chokepoint re-tune (`r19_chokepoint_retune`: 48-grid + catalog vs the shipped default over synthetic + imported + procedural-cave beds, maximin-over-regimes) crowns it: the ONLY config positive in ALL regimes ([+513/+315/+131], maximin +131, mean +319), while the June `a1-i6-tight-s2` profile measured NEGATIVE vs even the untuned default on synthetic (−839) and generated (−6) terrain. Both results stand for their eras: three months of kernel changes (Wave A/B, stall valves, exit-tile surcharge, rampart cover) reshaped the landscape, and the June single-bed open-terrain field could not see it. |
