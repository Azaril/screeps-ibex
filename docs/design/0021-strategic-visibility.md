...
scan reads intel but never asks for a refresh. This ADR records how visibility
actually works and what the strategic re-scouting layer on top of it must be.
...
- **Consumers register** via the `request_intel` pattern. Reference: [salvage.rs](../../screeps-ibex/src/operations/salvage.rs) `request_intel` (derelict confirm/recheck gates, `VISIBILITY_PRIORITY_MEDIUM`). Also `claim.rs` (`refresh_visibility_requests`, CRITICAL for unknowns / HIGH for stale candidates) and `sourcekeeperfarm.rs` (`STRONGHOLD_RESCOUT_INTERVAL = 1500`, LOW). Every consumer of strategic intel is a registrant — including `war.rs`.
...
rooms, `OBSERVE`-only is the correct registration.

## War-scan re-registration

[war.rs](../../screeps-ibex/src/operations/war.rs) offense scan: the stale-threat
branch (`last_seen > 200`) must not silently `continue`. It (a) does the
...
This is the minimal correct behavior for the offense scan; the strategic layer below
is what makes the cadence principled rather than incidental.

## The strategic re-scout layer

1. **A strategic re-scout *scheduler* (the WHAT/WHEN/HOW split).** Every
   consumer inlining its own cadence leaves no component owning "refresh these
   strategic rooms every N ticks." One owner (e.g. an `inject_strategic_rescout`
   pass in `ScoutOperation` next to `inject_flag_scout_requests`, or a small
   `StrategicVisibilitySystem`) walks the hot-tier rooms and re-pushes requests
   on an explicit cadence. Without it, the war-scan `MEDIUM`/`ALL` re-request rides the
   offense-scan cadence plus the 100-tick TTL — adequate, not principled.
2. **`OBSERVE`-only for observer-covered rooms (a prerequisite of the scheduler).**
   A helper registers/downgrades an entry to `OBSERVE`-only once a room is
...
3. **Frequency tiers**, by volatility: **hot** (active war/offense targets, SK rooms
   ...
   100t TTL); **candidate** (claim, mining-outpost) thousands of ticks — the existing
   cadence is correct; **lazy** (frontier) idle-scout opportunistic only — likewise
   already correct.
4. **Priority tuning.** The war-scan re-request uses `MEDIUM` (mirrors salvage, doesn't
   starve economy expansion which uses HIGH/CRITICAL). Escalate to HIGH for rooms with an
   *active* offense objective if offense should preempt expansion scouts.
5. **SK "stronghold cleared" probe.** `sourcekeeperfarm.rs` re-probes only while a
   stronghold is *present*; the symmetric probe is needed so the farm notices when it
   clears (register, don't self-scout).
6. **Observer capability/round-trip caveats.** No feedback loop validates that an RCL8
...
(unlimited-range observe) is out of scope here (ADR 0013 §6.3) — a future lever for
