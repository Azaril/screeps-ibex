...
## Progress + a scale-migration correction (2026-07-08)

**Done:**
- **A3** — the economic forming give-up (ADR 0042 §5), wired + private-validated (GaveUp firing).
- **A5 + A6** — `tower_refill_bid` connected onto the live path (`tower.rs`, `wall_repair.rs`): peaceful tower refill dropped from band `Low`=1250 (**the live S4 leak — above par**) to 500 (below par), hostile/emergency to `SURVIVAL_BID`. No WFV (numeric bid on an existing ticket shape).

**Scale-migration correction (affects sequencing).** Two of the lanes have different internal consistency:
- **Transfer lane is par-scale-consistent** — `sink_economics` (`refill_bid`, `buffer_deposit_bid`, `upgrade_bid`, `tier_to_bid`) all live at par = `STORAGE_BID` = 1000 (High=5000, refill up to 10000). So connecting its EV kernels does NOT hit a cross-scale problem; A1/A5/A6/A8 can proceed lane-by-lane. **BUT A1 is specifically the deferred ADR-0040 M5b "live-market rewire (bot analog of `market.rs`)":** `room_haul_demand` (`demand.rs`) is the **sim BASELINE arm's** tier generator (shared by `econ-eval/baseline.rs`), so it must NOT be changed to EV — the live bot must instead price its registrations via the market EV kernels (like the sim's MARKET arm / `market_adapter.rs`, whose `refill_bid` logic is currently test-only). Inputs (`lane_deficit_e` = Σ free spawn+extension energy, `next_body_cost_e`, `top_blocked_roi`) are reachable at the live registration; a first cut with `top_blocked_roi=None` already yields the deficit-scaled derived floor (the core win: a post-wipe empty lane ~10×par vs a near-full lane ~1.05×par, replacing the flat band). WFV: the tickets are already numeric-bid (M5a), so this is a **value change, not a shape change → likely NO WFV** (verify). A/B: the live arm diverges from the sim baseline arm — re-baseline the tournament.
- **Spawn lane MIXES scales** — `body_roi_milli` is par-scale (~12k for a good miner) while `SPAWN_BID_*` are band-scale ordinals (25k–100k). Converting ONE civilian role to `body_roi` makes it under-bid every band role. So **A2/A4/A7/A9/A10 (the spawn-EV batch) must be a COORDINATED migration** — all civilian spawn roles to `body_roi` at once, with the survival overrides (miner-bootstrap / downgrade → CRITICAL) as explicit floors above the par-scale bids, and the forming bid's reserved [HIGH,CRITICAL) band re-based onto the same scale. Larger and riskier than the transfer lane; do it as one unit with a full spawn-lane A/B re-sweep.

**Revised sequence:** A1 (transfer live-market rewire, lane-by-lane: refill → containers → controller) is the next tractable connection. A11/A12 (combat cleanup) are independent. The spawn-EV batch is the coordinated scale migration, last. Every item still converts a band to the ADR 0042 B1 pattern.
