...
| L4-activate | wire the offense `AttackFlag`/`Expansion` **enemy-force intel** into `PlayerRaid` | deferred | L4-doctrine | `AttackFlag` candidates are built OUTSIDE the threat scan with zeros (`war.rs:620`); cross-reference the flag room against `threat_rooms` to populate `enemy_force` **+ the towers** (`DefenseProfile`) — creeps-ONLY sizing would under-field vs a towered base (a regression). Then size on a player-raid bed before live. **L5** (N-blob) lifts the quad cap for big bases. |
