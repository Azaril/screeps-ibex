...
**Done so far (unification + debt):** L1 (registry + parity), L2 (`clear_force`), **L3a** (defender selection unified onto the registry; `DefenseEscalation` deleted), **L6a** (creep-clear bed + gate — `clear_force` clears 4/4), **L6b** (margin sweep — `COORDINATED_DPS_MARGIN` = 1.5 validated as the cliff-plus-buffer). The offline keystone (creep-clear sizing) is now **built, validated, and tuned**. **Next:** the behavior-changing rungs that wire it live — **L3b** (clear_force defender sizing, replaces spawn-path max-sizing), **L4** (`PlayerRaid`, also needs **L5** N-blob) — each validated on its bed before live, deploy-gated. **L7/L8** are independent low-risk unifications. **L6c** tunes the remaining `DoctrineParams` weights alongside the rungs that introduce them.
