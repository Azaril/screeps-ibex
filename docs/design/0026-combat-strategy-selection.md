...
**Done so far (unification + debt):** L1 (registry + parity), L2 (`clear_force`), **L3a** (defender selection unified onto the registry; `DefenseEscalation` deleted). **Next:** L6 (the tournament beds + `DoctrineParams` sweep) unblocks the behavior-changing rungs — L3b (clear_force defender sizing), L4 (`PlayerRaid`, also needs L5 N-blob), L7/L8 are independent low-risk unifications. Every behavior change validates on a bed (L6) before live, deploy-gated as ever.
