...
| L3 | **Rung 2 — `PlayerDefend`** doctrine; replace `DefenseEscalation::from_threat` (`war.rs:101`) | deferred | L2 + the defense path | sizes a defender to out-fight the incoming via `clear_force`; **always-fields** (you can't skip defending an owned room). Behavior change → wants its tournament bed first |
...
| L6 | **`DoctrineParams` tuning sweep** (the §9.8 dynamic weights) | deferred | the two near-winnability beds (Individual NPC + Coordinated player squad) | tunes `COORDINATED_DPS_MARGIN`, `coordination_dps_threshold`, `blob_escalation_parts`, `defend_size_curve`; bit-deterministic sim makes it cleanly tunable. **`COORDINATED_DPS_MARGIN` ships as a seed until this runs** |
| L7 | **SK keeper-suppression as a `SkSuppression` doctrine** | deferred | — | fold `sourcekeeperfarm.rs`'s inline `RequiredForce` sizing into the registry (it is a separate producer today; the sizing is already doctrine-shaped, R6 + R-attack) |
...
**Next rung = L3 (`PlayerDefend`)** — it has the cleanest wiring (defense always-fields, so no operator-intent/reachability conflict like L4) and retires `from_threat`'s three magic thresholds. L4 (`PlayerRaid`) follows once L5 (N-blob) lands. Both are behavior changes → validate on a bed (L6) before live, deploy-gated as ever.
