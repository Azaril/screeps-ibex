...
| L6b | **`DoctrineParams` tuning sweep** (the §9.8 dynamic weights) | deferred | L6a + a true-Individual (separate-enemy) bed | the bed exists; the sweep tunes `COORDINATED_DPS_MARGIN` (1.5 seed clears but slowly — kiters may want more), `coordination_dps_threshold`, `blob_escalation_parts`, `defend_size_curve` on the payoff metric. `COORDINATED_DPS_MARGIN` ships as a seed until this runs |
...
**Done so far (unification + debt):** L1 (registry + parity), L2 (`clear_force`), **L3a** (defender selection unified onto the registry; `DefenseEscalation` deleted), **L6a** (creep-clear validation bed + gate — `clear_force` clears 4/4, the keystone is validated). **Next:** **L6b** the `DoctrineParams` sweep (the bed exists; tune the margins) → then the behavior-changing rungs unblock: **L3b** (clear_force defender sizing), **L4** (`PlayerRaid`, also needs **L5** N-blob). **L7/L8** are independent low-risk unifications. Every behavior change validates on a bed before live, deploy-gated as ever.
