...
- **L8 — coordination is read from observed bodies** (§9.3's own principle): the scouted hostile creeps' OWNERS decide — all-NPC ⇒ `Individual`, any player-owned body ⇒ `Coordinated` (one player flips the room, per the Q1 asymmetry). The Q1 prior (Coordinated unless a positive NPC *source* signal) remains the fallback for a room with nothing observed.
