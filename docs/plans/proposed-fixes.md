...
> **Status: CLOSED 2026-06-12 — every row dispositioned.** Group A landed in Phase 0 (`d6eb76d`: IBEX-010/043/044/009/019/020/046 + the IBEX-013 assert/shrink — IBEX-045/050 went further than proposed: the `get_used_capacity(None)` double-count premise was disproven against engine source (engine-mechanics folklore row 26) and `HasExpensiveStore` deleted outright). IBEX-029 (`e00e3ac`) and IBEX-021 (`ca3b8e4`) landed in Phase 1. The Group B remainder landed 2026-06-12: **IBEX-018** as ADR 0012 M0+M1 (pure FairValue oracle + exposure ledger in `transfer/fairvalue.rs`, cost ceilings; history cache + ledger persist in dedicated seg 58 per operator direction — own version field, decoupled from `WORLD_FORMAT_VERSION`; the full TradePlanner stays Inc 7), **IBEX-040** in the `screeps-rover` leaf (resolver walkability consults the cached structure cost layer), **IBEX-048** as idle-suspend/recreate (depleted-mineral teardown + parent recreation gate). **IBEX-049 is DECLINED by operator decision** — see its section. This file is now historical record; live status is in the phase docs and [rewrite-plan §3](rewrite-plan.md).
>
> *(Original status follows.)* Proposed (for review). These are concrete, reviewable fix sketches — **NOT applied changes**. They are sourced from the review report's §4 Quick-Wins and the Low/Medium localized findings in [`../reviews/ibex-review-report.md`](../reviews/ibex-review-report.md). Each retains its `IBEX-NN` id so it cross-references the report, the Bug & Issue Register (§7), and the ADRs ([`../design/`](../design/)).
>
...
- **Status: DECLINED (operator decision, 2026-06-12).** Dropping serialized paths concentrates the entire fleet's repathing onto the post-reset tick: the rover's `needs_path` repath is *deliberately* unconditional and budget-exempt (so creeps never freeze), so every creep would run a full path search on exactly the tick that is already the most expensive — a CPU-overload vector traded for steady-state segment bytes that the IBEX-013 shrink and chunk watermark already keep comfortably bounded. Revisit only alongside staggered post-reset repathing, as part of the Inc-7-entry holistic serialization pass.
