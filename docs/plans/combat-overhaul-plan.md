...
- **P2.U8** — Single-creep behavior fixes (the SINGLE-* P1 set). **STARTED (operator chose U8 first, 2026-06-19):** **U8-1 DONE (`db37e1f`)** — edge-aware kiting: `kite_repulsors` adds a room-edge repulsor to the kite `Flee` when near an edge so a kiter rounds the room corner toward open space instead of fleeing into it and getting pinned at MOVE parity (the metric-surfaced finding). Byte-identical in open space; reuses the rover Flee (no one-off algorithm). Plus a metric refinement (`1ba009d`, operator): `melee_exposure_rate` now counts only adjacency to an **armed** enemy (`CreepFrame.attack_power`>0) — a disarmed adjacent creep isn't exposure. *Remaining U8:* the rest of SINGLE-* (focus/heal-self/engage-range polish). **High-conflict file** (`combat-decision/lib.rs`) — serialize against U9/U10. *Deps:* U5, U7. *Effort:* M.
