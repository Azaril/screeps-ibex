...
**Do-next ordering** (re-numbered sequentially): O5 is **resolved by dropping** power-bank farming (it was non-functional; real farming is the deferred ADR-gated workstream in §5(g)). So the offense finish is now **SK-DEADCODE (trivial) → G4-HEAVY (multi-squad mechanism, needs I1/I2) → O7 delete** (soak-gated; O7 no longer waits on power-banks). Then **H4/H5, I1/I2, W2–W4, S1/S2** as independent tracks. Full dependency-ordered queue in §5.
...
- `operations/attack.rs`: **`AttackOperation`** (full Recon/Prepare/Execute/Exploit/Complete state machine), `AttackReason`, `AttackPhase` — now launched **only** for the **heavy multi-squad player assault** (towers≥4, G4-HEAVY). PowerBank is no longer produced (O5 dropped); ThreatResponse/Expansion/ProactiveDefense/SourceKeeper/InvaderCreeps arms are dead (no producer).
- `operations/attack.rs:build_force_plan`/`plan_by_detected_threat` — the **heavy multi-squad assault planner** lives only here (G4-HEAVY-gated). This is the last thing keeping `AttackOperation` alive.
- `AttackReason::SourceKeeper` / `InvaderCreeps` / `PowerBank` arms — **DEAD** (nothing produces them; SK fully on `Farm{SourceKeeper}`, InvaderCreeps reconciled into remote-invader `Defend`, PowerBank dropped).
- `missions/attack_mission.rs`: **`AttackMission`** — multi-room siege layered-dismantle/wave-retry + the (non-functional) power-bank `Exploiting`/`power_bank_haulers` scaffolding; referenced by `jobs/squad_combat.rs` anchor-path branch. Kept only for the heavy assault until G4-HEAVY lands.
