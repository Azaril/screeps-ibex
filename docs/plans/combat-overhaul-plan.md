...
| WS | Rollup | Detail |
|---|---|---|
| **H** harness/engine | **PARTIAL** | H1/H2/H3 DONE. **H4 IN PROGRESS** — scripted opponents (rush/kite/turtle) + `run_engagement` self-play/adversarial runner + `world_from_units` composition builder landed (`screeps-combat-agent`, 25 tests); remaining H4 = drain opponent (needs a tower scenario) + the SVG replay scrubber + aggregated scoring (rolls into **H5** = `screeps-combat-eval` + server-parity + nightly gate, UNSTARTED). |
...
- **P2.H4 — IN PROGRESS.** ✅ Landed (`screeps-combat-agent`, commit `a1e517d`): scripted opponents `RushAgent`/`KiteAgent`/`TurtleAgent` (each a `TacticalAgent` emitting `MoveTo`/`Flee` goals routed through the same rover pathfinder the bot uses — pathfound, not raw directional; a wall-routing test proves it), `run_engagement` (head-to-head IbexAgent-vs-opponent runner → outcome + side-A cohesion), and `world_from_units`/`Unit` (composition builder so adversarial tests field our AI in arbitrary compositions). 25 tests. ⏳ Remaining: `DrainAgent` (needs a tower scenario), the SVG replay scrubber, and aggregated scoring/EXP-* harness (→ H5). *Why:* run the 0008a EXP-* register + close CP-H (military-score-on-combat is only kernel-tested). *Deps:* H1–H3. *Effort:* L (first increment done).
