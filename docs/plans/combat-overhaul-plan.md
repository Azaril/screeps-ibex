...
| WS | Rollup | Detail |
|---|---|---|
| **H** harness/engine | **PARTIAL** | H1/H2/H3 + **H4 DONE** (`screeps-combat-agent`, 28 tests): full scripted opponent roster (rush/kite/turtle/**drain**), `run_engagement` self-play/adversarial runner (+ scripted tower controller for tower scenarios, per-tick `CombatRecording` capture), `world_from_units` composition builder, and the **SVG replay scrubber** (`replay::to_svg` filmstrip; `examples/replay_demo`). **H5 UNSTARTED** = `screeps-combat-eval` crate: aggregated EXP-* scoring + sim-vs-server parity oracle + nightly seeded gate + byte-exact golden vectors. |
