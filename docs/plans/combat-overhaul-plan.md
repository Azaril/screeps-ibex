...
- **UNIFIED POSITION SELECTION (ADR [0019](../design/0019-combat-position-selection.md)) — designed 2026-06-19; Stage 0 + Stage 1 core + Stage 2 reachability core LANDED (`54cee73`, `1670723`, `1838e1a`); Stage 2-tail + Stage 3-4 pending.** Stage 2 shipped the operator's flood-fill: rover `reachability_from` (multi-source ticks-to-reach) + future-threat kite scoring (durable standoff). Stage 2-tail behavior DONE (`9d8d1f6`): wall-aware **g-cost cohesion** (centroid reachability flood). **Behavior content of Stage 2 + 2-tail complete** (future-aware kiting + wall-aware cohesion). **Stage 3a DONE (`28d6540`):** `PositionLayers` cache (operator's layer architecture) — the threat/centroid floods built once + reused across uses via `.fields()`; `plan_kite_anchor` byte-identical refactor. Next: **Stage 3b** — the unified utility (**T-POS subsumed**), build the compound-worst-case CPU sim bench first, then the integer `ThreatField` + `focus_damage` layers + signed `position_utility` + attack-positioning reusing `PositionLayers`; default-ON gated on the bench + 6 must-fixes. Generalizes the discrete flee/stand/close heuristics (incl. U8 kiting) **and** T-POS attack-positioning into ONE objective-aware per-tile utility over two shared maps (integer threat field + multi-source reachability flood-fill). HYBRID: one signed integer utility (flee/stand/close emerge as argmax under objective-selected weights) + 5 hard guards. Produced by an 11-agent ultracode deep-dive (verified file:line; reviews resolved). **Staged + flagged:** Stage 0 (prove tower-curve bit-identical) → Stage 1 (extract integer `ThreatField`, delete the `kite::tower_dps_at_range` duplicate, `combat-decision→combat-engine` dep + `boost` on `CombatBodyPart`, trivial-geometry fast-path — *pure refactor, no behavior change, byte-parity gated, kill-switch*) → Stage 2 (rover `reachability_from` + scratch reuse, U9 cohesion) → Stage 3 (unified utility + offensive positioning = **T-POS subsumed**, default-ON gated on a CPU compound-worst-case bench + 6 correctness must-fixes) → Stage 4 (incremental, measure-first). **No `WORLD_FORMAT_VERSION` bump.** *Deps:* U5/U6 harness (done), U8 (done); **T-POS is folded into Stage 3.** *Gated_on:* operator approval of Stage 0+1. *Effort:* L (staged).
