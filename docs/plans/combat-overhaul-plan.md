...
**RESUME POINT (2026-06-18):** after **P2.G4-O7** (offense legacy deleted; the G workstream is complete). Combat slice on master at Docker **WFV 13**. Host tests green: bot 157, rover 27, decision 41; wasm + clippy clean. **Last soak health (t≈454.7k): clean WFV-13 reset, CPU bucket FULL (10000), ~101 creeps / 6 rooms, `offense`+`defense` ON, no panics/deser/errors** — economy fully rebuilt and stable. **GAP: the world is peaceful, so the live objective/manager *combat* path (Dismantle core / Defend / Secure) is not yet behaviorally exercised** — this is the one remaining validation before M4 (needs a threat: wait/introduce one, or drive it via the H4 harness).
