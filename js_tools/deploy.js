...
const argv = require('yargs')
  ...
  .option('mode', {
    describe: 'build mode: debug or release',
...
function load_config() {
  ...
  return { branch, use_terser, extra_options, mode }
}
...
function run_wasm_pack(extra_options, mode) {
  const profile = mode === 'debug' ? '--dev' : '--release';
  let args = ['build', '--target', 'web', profile, 'screeps-ibex', '--out-dir', '../pkg', ...extra_options];
...
async function run() {
  ...
  await output_clean();
  console.log(`Building in ${config.mode} mode`);
  const build_result = run_wasm_pack(config.extra_options, config.mode);
