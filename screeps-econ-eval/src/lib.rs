...
//! excluding zero).
//!
//! Layout: [`layout`] realizes a captured foreman layout "as of RCL R"; [`movement`] is the
//! ANALYTIC fast movement tier (fatigue-exact single-creep traces from rover-eval's oracle
//! machinery, memoized — contention ignored BY DESIGN); [`baseline`] is the citation-transcribed
//! live policy (K1 demand / K2 selection / K3 repair admission / K4 spawning, each line cited to
//! its source) plus the S1-allowance arm; [`workers`] the harvester/hauler FSM shells;
//! [`scenario`] the Family-C catalog with per-bait NO-BAIT controls; [`oracle`] the T*
//! dependency-chain lower bound; [`metrics`] T_recover + diagnostics + the deadlock sentinel +
//! the gate; [`runner`] drives one scenario end-to-end. `bin/econ_bench` is the CLI.
//!
